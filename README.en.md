...
| Engineering baseline | **1171 tests passed** (author's machine, with the private artifacts present); **on a clean clone, measured 1125 passed / 46 skipped / 0 failed** (actually run on 2026-08-19, not derived from 1171 − 46) — all 46 skips are tests that need artifacts not distributed with the repo, or the ignored BFCL checkout. Ruff / `ruff format --check` / mypy (89 files) / `uv lock --check` / public-release audit passed in both of those environments on that date |

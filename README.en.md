...
> lock-in → explicit word-domain coverage (out-of-distribution absolute gate passed for the
> first time), sealed-boundary wobble → recorded honestly. The single source of truth for
...
0.00–0.107); v7 put that domain explicitly into training, **passing the OOD absolute gate
`ood_task_success_min` for the first time (0.6833 → 0.8333 ≥ 0.70)**, repairing the probe's
...
**Full detail in [`docs/RESULTS.md`](docs/RESULTS.md)** (where each number comes from, the
ugly number next to it, and what it cannot support). The single source of truth for
...
| Release decisions (120-task era) | First three **NO-GO**, fourth **GO** | Losses: `success_delta` −0.0333, then latency `p95_latency_ratio` **1.8774** at 120/120; the GO came from the **merged deployment form** of the same weights (ratio **1.1265**) — **attributed to deployment form, not the model** |
...
| Same era, round three (after enum-word coverage; final decision) | candidate **0.9797** / violations **5** / illegal calls **0** | **`ood_task_success_min` 0.8333 passed for the first time (≥0.70)**; probe near boundary (−1..−5) repaired to ≥0.875 per point; zero damage on dev and probe allow side; far side (−7/−10/−14) still collapsed (0.250–0.625) + sealed-boundary wobble → **NO-GO (11/12) + probe condition FAIL** — coverage generalization is graded |
...
| dev 198 tasks (v5/v6 era) | base **0.5404** → candidates **0.9949–1.0000** | dev was used for candidate selection — **selection bias** |
...
- **The final release candidate is `sft-008`**: the last decision was `NO-GO`
  ...
  and the OOD absolute gate passed for the first time — **the remaining gap is precisely
  attributed** (probe far-side mechanism undetermined; sealed-boundary wobble is run
...
**If those numbers look suspicious** (120/120, three metrics zeroed at once, 98.5%), that

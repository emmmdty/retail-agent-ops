...
| Engineering baseline | **1569 tests passed** (author's machine, with the private artifacts present); **on a clean clone, 1520 passed / 49 skipped / 0 failed** (re-verified at the v7 close) — all 49 skips are tests that need artifacts not distributed with the repo, or the ignored BFCL checkout. Ruff / `ruff format --check` / mypy (116 files) / `uv lock --check` / public-release audit passed in the author environment |

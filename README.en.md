...
| Engineering baseline | **1540 tests passed** (author's machine, with the private artifacts present); **on a clean clone, 1491 passed / 49 skipped / 0 failed** (measured on 2026-09-07; on 2026-08-27 the previously published 1306/46 was measured on `88ccabb` and matched the derived value) — all 49 skips are tests that need artifacts not distributed with the repo, or the ignored BFCL checkout. Ruff / `ruff format --check` / mypy (116 files) / `uv lock --check` / public-release audit passed in the author environment |

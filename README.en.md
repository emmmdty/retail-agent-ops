...
| Engineering baseline | **1350 tests passed** (author's machine, with the private artifacts present); **on a clean clone, 1304 passed / 46 skipped / 0 failed** (measured on 2026-08-27; the same day the previously published 1238/46 was measured on `88ccabb` and matched the derived value) — all 46 skips are tests that need artifacts not distributed with the repo, or the ignored BFCL checkout. Ruff / `ruff format --check` / mypy (89 files) / `uv lock --check` / public-release audit passed in the author environment |

...
| Engineering baseline | **1146 tests passed** (author's machine, with the private artifacts present); **on a clean clone, measured 1101 passed / 45 skipped / 0 failed** (actually run on 2026-08-19, not derived from 1146 − 45) — all 45 skips are tests that need artifacts not distributed with the repo, or the ignored BFCL checkout. Ruff / `ruff format --check` / mypy (89 files) / `uv lock --check` / public-release audit passed in both of those environments on that date |

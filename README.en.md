...
| Engineering baseline | **1226 tests passed** (author's machine, with the private artifacts present); **on a clean clone, 1180 passed / 46 skipped / 0 failed** (derived on 2026-08-23 as 1226 − 46; the previous 1174/46 was actually measured on 2026-08-22 — the newly added parser regression tests do not depend on private artifacts) — all 46 skips are tests that need artifacts not distributed with the repo, or the ignored BFCL checkout. Ruff / `ruff format --check` / mypy (89 files) / `uv lock --check` / public-release audit passed in the author environment |

...
| 工程基线 | **1349 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1303 passed / 46 skipped / 0 failed**（2026-08-27 实跑；同日在 `88ccabb` 上实跑复核了上一轮公布的 1238/46，与当时的推算值一致）；46 条跳过全部因为要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(89 源文件) / `uv lock --check` / 公开发布审计在作者环境通过 |

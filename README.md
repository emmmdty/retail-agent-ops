...
| 工程基线 | **1226 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1180 passed / 46 skipped / 0 failed**（2026-08-23 由 1226 − 46 推算；上一轮 1174/46 为 2026-08-22 实跑值，本轮新增的解析器回归测试不依赖私有产物）；46 条跳过全部因为要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(89 源文件) / `uv lock --check` / 公开发布审计在作者环境通过 |

...
> 措辞域锁定 → 词域覆盖（分布外绝对门 0.6833 → 0.8333 翻正）、封存边界摆动 → 如实入账。
...
[English](./README.en.md) ｜ [产品规格](./SPEC.md) ｜ [模型卡](./docs/MODEL_CARD_sft-006.md)（历史候选卡片；最终候选 `sft-008` 的身份与失败模式见 [结果完整版](./docs/RESULTS.md) 与[台账](./docs/HOLDOUT_LEDGER.md)） ｜
...
在这三轮之前还有一次判定，最能说明门禁的价值：门禁升级到 v1.3 十二门后的首次真实判定
（观测 7，2026-09-06）把与此前 GO 同值的读数（117/120、政策违规 2、非法调用 0——
在 v1.0/v1.1 口径下是 GO）按新加的绝对安全门判成了 NO-GO。**门禁升级，结论跟着变，
这正是门禁在干活。**
...
`ood_task_success_min` 由观测 9 的 FAIL（0.6833）翻正为 0.8333（≥ 0.70）——此前
观测 7/8 该门曾以 0.9833/1.0000 通过**、探针近边界修复到逐点
...
**完整展开见 [`docs/RESULTS.md`](docs/RESULTS.md)**（含 v1.3 十二门口径；每个数字怎么来的、
旁边那个不好看的数、它不能支持什么）。观测次数与逐次读数的唯一事实源是
...
| 发布门禁判定（120 条口径） | 前三次 **NO-GO**，第四次 `GO` | 第一次输在 `success_delta` −0.0333；第二、三次候选做到 120/120 却输在延迟 `p95_latency_ratio`（**1.8774 / 2.0250**，第二、三次观测）；拿到 GO 的是**同一份权重的合并部署形态**（比值 **1.1265**）——**GO 归因于部署形态，不是模型** |
...
| 同口径第三轮（枚举词域覆盖后，最终判定） | candidate **0.9797** / 违规 **5** / 非法调用 **0** | **`ood_task_success_min` 由观测 9 的 FAIL（0.6833）翻正为 0.8333（≥ 0.70；此前观测 7/8 该门曾以 0.9833/1.0000 通过）**、探针近边界（−1..−5）修复至逐点 ≥0.875、dev 与探针放行侧零损伤；探针远端（−7/−10/−14）仍塌（0.250–0.625）+ 封存边界摆动，判定 **NO-GO（11/12）+ 探针条件 FAIL**——词域覆盖的泛化是分级次的 |
| 政策边界探针（15 偏移 × 措辞域，每点 n=8） | 修复前拒绝侧 7 点 **0.00–0.107** → 修复后近边界 **≥0.875** | 每点 n=8 的 95% CI 宽度约 ±35pp：**足以看曲线形状，不足以给单点排序** |
| dev 198 条（v5/v6 口径） | base **0.5101（v5）/ 0.5404（v6）** → 候选 **0.9949–1.0000** | dev 已被用于候选选择，**带选择偏差** |
...
| teacher 数据采集 | 通过率分批：早期两批 **87.9% / 99.2%**；v5 批次 **96.3%**；v6 批次总体 **579/588 = 98.5%** | 正式批次成本 **$0.0559** 级（含采集前枚举域断言，返工 0 条） |
| 工程基线 | **1570 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1518 passed / 52 skipped / 0 failed**（收尾分层复核）；跳过全部因为要读不随仓库分发的私有产物。Ruff / `ruff format --check` / mypy / `uv lock --check` / 公开发布审计在作者环境与 CI 通过 |
...
- **最终发布候选是 `sft-008`**：最后一次判定 `NO-GO`（11/12 + 探针条件 FAIL），
  候选未更换。12 门里有 11 门 PASS 且分布外绝对门由观测 9 的 FAIL（0.6833）翻正为
  0.8333，**剩余的缺口被精确归因**
...
**如果你觉得这里几个数高得可疑**（120/120、三项指标同时为 0、v6 批次的 98.5%），

...
| 工程基线 | **1219 tests passed**（作者环境，私有产物齐全） | **干净 clone 上实测 1173 passed / 46 skipped / 0 failed**（2026-08-20 实跑，不是由 1219 − 46 推算）；46 条跳过全部因为要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(89 源文件) / `uv lock --check` / 公开发布审计在这两种环境下、于该日期通过 |

...
| 工程基线 | **1284 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1238 passed / 46 skipped / 0 failed**（2026-08-24 由 1284 − 46 推算；上一轮 1180/46 为 2026-08-23 推算值）；46 条跳过全部因为要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(89 源文件) / `uv lock --check` / 公开发布审计在作者环境通过 |

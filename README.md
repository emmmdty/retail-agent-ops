...
| 工程基线 | **1540 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1491 passed / 49 skipped / 0 failed**（2026-09-07 实跑；2026-08-27 曾在 `88ccabb` 上实跑复核 1306/46）；跳过全部因为要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(116 源文件) / `uv lock --check` / 公开发布审计在作者环境通过 |

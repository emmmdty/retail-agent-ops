...
| 工程基线 | **1094 tests passed**（作者环境，私有产物齐全）；**干净 clone 上是 1049 passed / 45 skipped / 0 failed**——跳过的 45 条全部因为它们要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(86 源文件) / `uv lock --check` / 公开发布审计在两种环境下都全绿 |

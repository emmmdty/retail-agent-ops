...
| teacher 数据采集（DeepSeek） | 通过率 **99.2%** | 正式批次成本 **$0.0559**（含先行批次合计约 $0.111，两批不得混为一谈） |
| 工程基线 | **1435 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1397 passed / 38 skipped / 0 failed**（2026-09-04 实跑；2026-08-27 曾在 `88ccabb` 上实跑复核 1306/46）；46 条跳过全部因为要读不随仓库分发的私有产物或 ignored 的 BFCL checkout。Ruff / `ruff format --check` / mypy(113 源文件) / `uv lock --check` / 公开发布审计在作者环境通过 |

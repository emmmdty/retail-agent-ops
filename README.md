...
| 工程基线 | **1569 tests passed**（作者环境，私有产物齐全） | **干净 clone 上 1520 passed / 49 skipped / 0 failed**（v7 收口复核）；跳过全部因为要读不随仓库分发的私有产物。Ruff / `ruff format --check` / mypy / `uv lock --check` / 公开发布审计在作者环境与 CI 通过 |
...
**说明**：本仓库另有一层**作者本地工作文档**（求职材料与 agent 运维记忆），不随仓库

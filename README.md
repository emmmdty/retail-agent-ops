...
第 5 次是当前最终候选。**它不是满分**：117/120，且政策违规从 0（`sft-006`）回到 **2**——
这正是 R6 那笔交换在模板内的代价，见下文「泛化修复」。
...
| 工程基线 | **1069 tests passed**；Ruff / `ruff format --check` / mypy(86 源文件) / `uv lock --check` / 公开发布审计全绿 |

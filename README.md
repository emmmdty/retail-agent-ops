...
> 走通**三次**，**三次发布结论都是 `NO-GO`**，阈值一个字未改（有测试锁定）：
> 第一次输在 `success_delta`（0.7833→0.7500）；第二、三次候选在 120 条上做到
> **120/120**、`success_delta` **+0.1417**、违规与非法调用清零，但输在延迟门禁。
> 第三次同时证明：那笔延迟代价**主要来自部署形态而非模型**——把 LoRA 合并回基座后，
> 同样 120/120、调用次数一模一样，p95 比值 2.03 → **1.21**。
> 每次都据此回滚加载纯基座。

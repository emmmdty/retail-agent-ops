...
## Current Task: DPO 主线判读完成（修坏）→ 下一决策门 B-4 数据重建立项（2026-09-06）

**上一任务（DPO 主线 + 并行项）已完成**：A-0 三方案点确认 → 预注册（`eeec309`，
两次修订留痕）→ 采样器/训练管线 TDD → R11-1 采样（27 对全在 `offset −14`、
...
**下一决策门**：B-4 合并数据重建立项
（`docs/PROPOSAL_DATA_REBUILD_B4.md`：难度分层切分 + max_steps 6 + reason
口径 A 合并为一次 R2 量级重建；三个决策门——立项与否 / teacher 预算 / 与 DPO
先后——待用户裁决）。**不与其它工作同窗口执行**（它会使既有判读口径失效）。
绝对门（封存集零违规）现状：DPO 本设计证伪入账，无在途修复尝试。

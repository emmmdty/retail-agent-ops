# Task Plan: RetailAgentOps

## Goal

在 12 周内交付可公开、可复现、可面试解释的零售工具 Agent 单卡适配与发布流水线。
R0–R5 已完成并封存（tag `v1.0-r5`），R5 收尾摘要见 `progress.md`。

## Current Phase

**R6 泛化修复**（用户于 2026-08-16 追加，要求把项目推到 9 分以上）。

外部审阅两轮给到 8.5/10，并明确指出天花板在哪：
> 「8.5 到 10 的差距不在工程，在**有一个扛得住分布漂移的结果**。」

R6 的目标就是那个结果。**不是再补文档、再加测试。**

## Current Task：R6 泛化修复

### 诊断（已完成，这是本轮的出发点）

分布外读数按子类拆开后，机制是清楚的：

| expression 子类 | 零训练基座 | 合并候选 |
|---|---|---|
| `code_switch` | **1.0** | **0.0** |
| `colloquial` | **0.5** | **0.0** |
| `typo` / `terse` / `greeting_noise` | 0.0 | 0.0 |

**训练把基座本来会做的两类打没了**，20 条失败**全部**是 `premature_final_response`
（正是 R3 那个"说完就停"）。

**机制**：240 条训练数据全部来自 `formal_tasks._user_request` 的 12 句模板，
且 12 句**全部**是"请核实 / 请检查 / 请查询…"的书面祈使句。模型学到的是
**表面形式 → 动作**的触发器，而不是**语义 → 动作**。换一种说法（口语、错别字、
中英夹杂、极简）触发器不响，模型就退回"用自然语言解释"。

### 输入

- 基线 `bfa5c8d`（`v1.0-r5`），948 tests passed，全部质量门通过；
- **`.env` 里有可用的 DeepSeek 凭据**（本轮已 smoke 验证 HTTP 200）——
  这解锁了此前被记为「未做：两台机器都没有 `TEACHER_LLM_*` 凭据」的 LLM 改写路径；
- 已冻结的四条硬契约（`GATE_IDS`、dev `PAIRING_FIELDS`、`SEALED_PAIRING_FIELDS`、
  40/10/20 配额 + `dataset_version`）；
- 240 条已验证 teacher 轨迹（`train-export-004` 的来源）。

### 输出

| 编号 | 内容 | 资源 |
|---|---|---|
| T1 | **措辞池**：LLM 生成 + 双向校验 + 按哈希确定性三分（训练增强 / OOD-dev / OOD-sealed） | API |
| T2 | 导出侧支持 paraphrase 增强（`sft_paraphrase_*` 两个键，沿用「声明期望哈希」的形状） | CPU / TDD |
| T3 | **OOD v2 任务集**：复用 v1 的场景与判定逻辑，只把请求文本换成措辞池 | CPU / TDD |
| T4 | 重训 + 评测：dev 60（回归）、OOD-v2-dev（迭代） | GPU |
| T5 | **OOD-v2-sealed 一次性判定** + OOD v1 独立迁移检查 | GPU |
| T6 | 演示视频（终端录制 → 可播放文件） | CPU |
| T7 | 「数字是否高到可疑」审视：逐个满分/接近满分的读数配上下文与限度 | CPU |
| T8 | 文档、`PROJECT_LOG`、独立面试官第三轮 | CPU |

### 方法学守则（本轮成败的关键，违反即结论作废）

1. **措辞池按 `sha256(措辞 + 固定盐)` 确定性三分**，不是人工挑选。
   训练增强用的措辞与评测用的措辞**互不相交**，且由代码断言。
2. **OOD-v2-sealed 只观测一次**，在 OOD-v2-dev 上的迭代全部结束、代码冻结并提交之后。
3. **OOD v1（作者手写那 60 条）本轮不得用于任何选择**——它是**独立迁移检查**，
   作者不同、生成过程不同，只在最终一次性报告。用它选候选就等于污染它。
4. **封存 120 条 holdout 默认不碰**。若最终候选在 OOD-sealed 上确实站住，
   再决定是否消耗第五次观测——那是发布判定，不是调参输入。
5. **不下调任何门禁阈值**；不改 `dataset_version` / 40/10/20；不改 v1 bundle。
6. **改写只动 user 的第一条消息**，assistant 的工具调用与最终状态一个字不动——
   否则改的就不是「说法」而是「任务」。改写后必须过**语义回环校验**
   （让模型把改写反分类回场景，不一致即丢弃）。

### 失败模式（实施时主动防御）

1. **拿 OOD 集合调参**：这是本轮最容易犯、也最致命的错。用 sealed 分片 + 守则 2/3 挡；
2. **改写把任务改了**：例如把「退款」写成「换货」、丢掉 order_id、把「重试一次」删掉 →
   语义回环校验 + 结构校验（必须含 `{order_id}` 占位符）+ 逐场景配额检查；
3. **增强只是把同一句复制 N 遍**：措辞池内需去重，并断言每条 task 的改写互不相同；
4. **修好了 OOD 却打坏了 dev**：dev 60 必须同时报告，退化即如实记录；
5. **API 成本失控**：先跑 smoke 估算 token，再跑全量；两批成本都记账；
6. **「9 分」变成自我宣布**：最终由不带上下文的外部 reviewer 判定，不由我判定。

### 最终候选的选择规则（**在看到 sft-008 任何读数之前写定**）

两个候选：`sft-007`（增强 + 保留 oversample）与 `sft-008`（增强 + 去掉 oversample）。
选择只用 **OOD-v2-dev** 与 **dev 60 回归**，两者都不封存。

已知 `sft-007` 的读数：OOD-v2-dev **0.9667**；dev **58/60，其中 2 次政策违规**
（均为 `refund_denied_window` / `refund_not_eligible`）。

规则：

1. `sft-008` 的 dev 政策违规 **= 0**，且 OOD-v2-dev 相对 sft-007 下降 **≤ 0.05**
   → 选 `sft-008`。理由：政策违规是安全类失败，比同量级的任务失败更严重。
2. `sft-008` 的 OOD-v2-dev 相对 sft-007 下降 **> 0.05**
   → 选 `sft-007`，并把「去掉 oversample 会牺牲分布外收益」作为结论写出来。
3. `sft-008` 的 dev 政策违规 **仍 > 0**
   → 假设被证伪，选 `sft-007`（OOD 更好），并把那 2 次违规作为**代价**如实写进结论。

**无论选中哪个，两个候选的完整读数都会并列出现在文档里。**

**封存分片只观测一次**，运行内容固定为三个：零训练基座、`sft-006`（未合并）、
以及上面规则选出的那一个。**不会两个候选都跑封存**——那等于用封存分片做选择。

### 规则的实际应用（2026-08-17，读数落地后）

`sft-008`：OOD-v2-dev **0.9833**（sft-007 是 0.9667），dev **58/60，pv 仍是 2**，
且两个候选的 dev 失败签名**完全相同**——都是 2 条
`refund_denied_window` / `refund_not_eligible`。

**假设被证伪**：去掉 oversample 没有消掉那 2 次违规。因此违规**不是 oversample 造成的**，
而是**措辞增强本身的代价**——这个结论比原假设更干净，因为它由两个只差 oversample 的
候选共同支持。

**规则第 3 条有一个被证伪的前提。** 原文是「假设被证伪 → 选 `sft-007`（OOD 更好）」，
括号里那句是我写规则时的预期，而实测 `sft-008` 的 OOD **更好**、dev **完全相同**。
按字面执行会选中一个被严格支配的候选——那不是纪律，是仪式。

**因此选 `sft-008`**，理由是它在两个判据上一个更好、一个持平（严格支配）。
**这次偏离字面规则的事实、以及偏离的理由，写在这里而不是只在对话里说过。**
下次写规则时的教训：别在规则里塞入对结果的预期，那会让规则在预期落空时自相矛盾。

**封存分片的运行内容据此固定为**：零训练基座、`sft-006`（未合并）、`sft-008`。
`sft-007` **不跑封存**。

### 第五次封存 holdout 观测：内容与判读**在跑之前写定**（2026-08-17）

**为什么要跑。** 项目的整个主张是 `build → evaluate → release → serve` 这条链路，
而**最好的候选 `sft-008` 从未经过发布门禁**——它只有 dev 与 OOD 读数。
「你修好了泛化，那它过不过你自己的门禁？」这个问题现在的答案是「我没测」，
这是本项目当前最大的结构性短板，比任何文档问题都严重。

**为什么现在跑而不是更早。** 封存 holdout 的价值来自很少被观测。前四次每一次都对应
一次正式发布判定；第五次同样是一次正式判定，不是调参输入。**跑之前 R6 的全部迭代已结束、
代码已冻结提交（`6b9e165`）。**

**运行内容（三个，固定，不得增减）**：

1. `holdout-base-005`：零训练基座（必须重跑——`code_commit` 与 `uv_lock_sha256` 都在
   `SEALED_PAIRING_FIELDS` 内，第四次的 base 证据与新 commit 不可配对）；
2. `holdout-merged-candidate-005`：**`sft-008` 的合并部署形态**；
3. `release`：v1.0 与 v1.1 两套口径各出一份判定。

**为什么候选侧用合并形态**：第三、四次已经证明未合并形态的延迟代价是部署实现开销
而非模型能力（单次调用 1.985×，调用次数只增 1.146×）。用未合并形态跑等于明知会输在
一个已归因清楚的实现细节上。**这个选择在跑之前写定，不是看到结果后挑的。**

**判读（同样事先写定，三种都要如实写）**：

- **GO**：项目拿到一个**既过门禁又在分布外站得住**的候选。这是最强的结果，
  但仍不等于「可以上线」——OOD v1 的 `scenario_ood` 0.60 与 dev 的 2 次政策违规照写。
- **NO-GO / 延迟类**：与前三次同因，说明合并形态的余量（此前只有 1–3%）没撑住。
  如实记，且**不得再改部署形态去凑**——那会变成对着封存集调形态。
- **NO-GO / 任务或安全类**：说明 R6 的措辞增强在模板内是有代价的
  （dev 上那 2 次政策违规在 120 条上放大了）。**这个结果对项目同样有价值**——
  它会把「R6 修好了泛化」修正为「R6 用模板内的安全性换了分布外的鲁棒性」，
  与 R4 那次的权衡结构完全对称，必须照写。

**无论哪种结果，第五次观测都会写进 `HOLDOUT_LEDGER.md`，且不再有第六次。**

### 验收命令

```bash
.venv/bin/pytest -q
.venv/bin/ruff check . && .venv/bin/ruff format --check .
.venv/bin/mypy
env -u UV_INDEX_URL -u UV_DEFAULT_INDEX uv lock --check
git diff --check
.venv/bin/python scripts/ci/verify_qualification_chain.py
.venv/bin/python scripts/ci/audit_public_release.py
```

### 授权状态

GPU **是**、**商业 API 是**（用户 2026-08-16 授权，`.env` 已有凭据）、
模型下载 **否**、封存 120 holdout **暂缓**（见守则 4）、公开发布推送 **否**、
新依赖 **允许**（用户授权；从中国镜像下载）。

### 进度

- [x] 0. 诊断：按子类拆开 OOD 读数，定位机制
- [x] 1. `.env` 凭据验证（HTTP 200）与 R6 计划
- [x] 2. T1 措辞池（267 条，$0.0061，哈希三分 147/59/61）
- [x] 3. T2 导出侧 paraphrase 支持（sft 400→1600，说法 42→184 种）
- [x] 4. T3 OOD v2 任务集（dev/sealed，oracle 可解性各 60/60）
- [x] 5. T4 重训 ×2 + dev/OOD-v2-dev 评测 ×7
- [x] 6. T5 **OOD-v2-sealed 一次性判定**（0.7167 / 0.7333 / **1.0000**）
      + OOD v1 迁移检查（`expression_ood` **0.00 → 1.00**，总分 0.8667）
- [x] 7. T6 演示视频（`docs/media/demo.mp4`，81 秒，捕获与渲染两步分离）
- [x] 8. T7 「数字可疑度」审视（`docs/READING_THE_NUMBERS.md`）
- [x] 9. T8 文档全线贯通 + `PROJECT_LOG` 两条 + ADR 0005
- [ ] 10. 独立面试官第三轮审核

## Task Rules

- 本文件只跟踪当前任务；长期阶段状态以 `docs/EXECUTION_PLAN.md` 为准。
- 新任务开始时重写 Current Task，保留已完成任务的摘要到 `progress.md`。
- 输入、输出、非目标、失败模式和验收命令不完整时不得开始实现。
- GPU、API、数据下载和公开发布必须显式标注授权状态。

## Errors

| Date | Error | Resolution |
|---|---|---|
| 2026-08-16 | R5 重建复验首次起跑漏了 `--input_dir`，CLI 硬失败退出 | 私有训练数据根是 `data/private/retail_ops/v1/r2/retail_ops_v1_r2_20260722`；补参数后重跑 |
| 2026-08-16 | 按外部审阅修 teacher/训练数字时只改了主表，漏了 5 处 | 手工同步同一组数字必然漏；改为绑到可执行校验（`test_the_two_teacher_batches_are_never_conflated` 等） |

（R0–R4.5 的历史错误台账已归档到 `progress.md`。）

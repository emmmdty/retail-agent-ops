...
- [x] 18. **vLLM 走完整 evaluate 路径（dev + OOD）已完成**（`docs/ENGINE_SUBSTITUTION.md`、
      LOG-20260816-03）。契约一字未改——让 vLLM 也跑 bnb NF4，引擎成为唯一变量。
      dev/候选 1.0000 vs 1.0000、OOD/候选 0.5833 vs 0.5833 逐类别全同，
      但 **OOD/零训练基座 0.2167 vs 0.2333 不一致**。吞吐 ~4.85×。
- [x] 19. **证据补记真实运行环境**（LOG-20260816-04）：新增 `inference_engine` 与
      `runtime_env_sha256`，补上 `uv_lock_sha256` "只锁一个文件"的盲区。
      48 份已有证据全量复算 0 不匹配。**封存证据未改**（需 v1.2，独立决策）。

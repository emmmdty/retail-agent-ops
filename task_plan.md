...
## Current Task: v7 迭代——已收口（2026-09-08，判定 NO-GO 11/12 + 探针条件 FAIL，候选不变）

**判定与归宿**：观测 10（封存 246 条，与观测 9 同一封存集）candidate（合并形态）
0.9797（241/246）/pv5/inv0——唯一失败门 `policy_violation_count_max`（5>0，4×
`cancel_denied_recent` 新失败点 + 1× 观测 9 同款残留），另预注册探针条件独立 FAIL
（远端 −7/−10/−14 三点 0.250–0.625 < 0.875）。**11/12**：v6 的失败门
`ood_task_success_min` 历史首过（0.8333 ≥ 0.70），探针近边界（−1..−5）修复至
≥0.875，dev/ALLOW 侧零损伤——「枚举词 deny 缺失」根因链被预测-验证闭环证实，
干预在靶面按设计生效但不足以过绝对门。按 A-0 三分支第二分支（NO-GO）如实收官：
**候选保持 `sft-008`**，不重跑、不换素材、不向封存集重掷骰子。
formal-release-010（`report_id 29e8b8d6…`）；LOG-20260908-02；HOLDOUT_LEDGER 观测 10。

**本轮门级/方法论结果**：(1) 根因链预测-验证闭环——显式词域覆盖把靶面（枚举词域）
从 basin 运气变成被训练出的能力（近边界 + OOD 绝对门）；(2) 覆盖泛化的分级性——
近边界修复 ≠ 远端修复，「档位密度不足」解释被排除，远端机制未定；(3) 绝对门的
抽签成分实测——两轮重训失败点不固定（观测 9 `refund_denied_window` ↔ 观测 10
`cancel_denied_recent`），「修好已诊断失败面」与「封存零违规」是两个独立命题。
下一迭代候选入口：远端塌方机制诊断、封存边界摆动方差治理，均需单独预注册。

执行入口（历史）：`docs/handoffs/2026-09-08-v7-wording-coverage-execution-prompt.md`
...
**运行清单实际执行记录（2026-09-08，与预注册声明一致）**：
V7-2 TDD 实现（`fadaa48`）→ V7-3-1 导出（2352 前缀逐字节相同 + 426 追加行，
allow 210/deny 216）→ V7-3-2 dev_sft 沿用 v6 → V7-4-1 训练（522 步
loss 1.639→0.016）→ merge（`a121894a…`）→ V7-4-2..7 级联 9 运行同 commit
（`4e3b70d6` 系，零 FAILED）→ V7-4-8 release v1.3（`formal-release-010`，NO-GO）。
...
`ood_task_success_min`（0.6833<0.70）双门 FAIL，另预注册探针条件独立 FAIL
（DENY 侧 7 点 0.00–0.107）。按 A-0 三分支第二分支（NO-GO）如实收官：
**候选保持 `sft-008`**，不重跑、不换素材、不向封存集重掷骰子。
LOG-20260908-01；HOLDOUT_LEDGER 观测 9；PITFALLS #27。

**本轮真正产出的三个门级/方法论结果**：(1) V6-2b 解析器协议容忍——v5 唯一失败门
`invalid_call_count` 归零（评测语义版本化机制首用并验证）；(2) rtc_stepwise 修复
——teacher 分桶 0.643→1.000、总体 98.5%，采集前枚举域断言上线；(3) 负结果入账——
「该拒绝」判定可以是措辞域锁定的（枚举词域从未入训练分布，basin 性质），
「同源评测面高估」第三次实证。下一迭代候选方向：词域覆盖（枚举词 deny 请求入训练
+ 双词域判读），待单独预注册与用户决策。

执行入口（历史）：`docs/handoffs/2026-09-07-v6-iteration-execution-prompt.md`

### 上一任务：v6 迭代预注册与执行记录（2026-09-07/08，已完成）

**A-0 裁定（2026-09-07，用户逐项确认，已履行）**：

1. **范围**：先诊断后定——V6-1 空生成根因诊断先行（不占封存观测，禁用封存集
   提示词作诊断输入）；诊断指向数据侧可修项则一并入 v6，指向引擎栈则单独评估。
   v6 数据侧确认单变量 = rtc_stepwise 请求修复（V6-2）。
2. **dataset_version**：`retail_ops_v6_20260907`（按冻结日命名）。
3. **判读规则**：v1.3 十二门 + 探针每点 ≥ 0.875；三分支完整划分；v1.4 不启用。
4. **（2026-09-07 修订）缺闭合标签缺陷处置 = 解析器协议容忍**（新 parser_id
   版本键控，最窄条件，诊断计数不进门禁，老证据零影响）。

**判读规则（v6 正式稿，已按观测 9 履行）**：

1. **GO**：v1.3 十二门全 PASS 且探针 15 偏移每点 ≥ 0.875 → 换候选 `sft-v6-001`。
2. **NO-GO**：十二门任一 FAIL 或探针任一点 < 0.875 → 不换候选；如实收官；失败门
   只作事实记录；不重跑、不换素材、不向封存集重掷骰子。**← 落点**
3. **边界组合**：判定面不成立 → 升级用户。未触发。

**运行清单（v6 实际执行记录；产物目录与预注册声明一致）**：
V6-1a 静态诊断（分类修正）→ V6-1b 重放取消（类被解析器容忍确定性消除，
预注册声明未运行）→ V6-2/V6-2b TDD 实现（`dca981e`）→ V6-3-0 登记（`f44db3d`）
→ V6-3-1 冻结+覆盖表（`c07e5b4`）→ V6-3-smoke（18/18）→ V6-3-2 全量
（588/588，579 接受 98.5%，rtc 42/42）→ V6-3-3/4 导出（2352/198 行）→
V6-4-1 训练（441 步 loss 1.2525→0.0464）→ merge（`8f2c0591…`）→ V6-4-2..5
评测级联 → V6-4-6 观测 9 → V6-4-7 release v1.3（`formal-release-009`，NO-GO）。

**输入/输出/非目标/影响文件/验收命令**：见预注册原文（`696ad81`+`5810959`）；
全部验收命令已执行（1544 passed 作者环境 + 干净 clone 1495/49 + 全门绿）。

### v6 预注册原文（存档：判读规则/运行清单/输入输出——运行清单是声明守卫的绑定对象，r13-v6 目录已落盘，不得删除）

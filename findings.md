...
### gpu-5090 的 NVIDIA 驱动在本轮中途卡死（2026-08-27 15:38，环境故障非代码问题）

GPU 阶段第二次启动跑到训练时报 `QLoRA-SFT 必须看到且只看到一张 CUDA GPU`
（`training/sft.py:369`）。**不是多卡也不是没卡，是驱动卡死**：

| 检查 | 结果 |
|---|---|
| `nvidia-smi -L`（20s 超时） | 无输出，进程进入 **D 状态**永不返回 |
| `import torch; torch.cuda.is_available()`（300s 超时） | 退出码 124，超时 |
| `ps` D 状态进程 | 4 个：两个 `Xorg`（15 天 / 10 天）+ 我发起的两个 `nvidia-smi` |
| load average | **9.95 / 10.89 / 12.17**，而所有进程 %CPU ≤ 0.7 |
| `nvidia-smi --query-gpu` 最后一次成功时 | 显存占用 7027 MiB，但 `--query-compute-apps` 返回 `[N/A]` |

D 状态计入 load average 却不消耗 CPU——「load 10+ 而 CPU 全空」正是驱动卡死的指纹。
显存显示被占用但占用者不可归属，说明那个上下文已经脱管。

**时间线**：15:03–15:16 训练成功（说明修好的代码是能跑的），15:26 第二次启动，
15:38 撞上守卫。卡死发生在 15:16–15:38 之间。

**没做的事**：没有去杀 PID 3534676（`tongjiakai` 自己的另一个作业，已跑 2 天 3 小时）；
没有尝试 `--gpu-reset` 或重启——gpu-5090 多人共用（`xuhui`/`server` 账户在线），
这两项都影响其他用户，属于用户决策。

**代价为零**：采集证据（159 条）、SFT 导出、manifest 全部已落盘保留，
重跑时 teacher 阶段整体跳过、不再计费，从训练阶段续上。

**教训写进交接文档 §5.5**：先用 `timeout 20 nvidia-smi -L` + D 状态计数分清
「真的多卡/没卡」和「驱动卡死」，两者处理方式完全不同；卡死时**每多试一次
就多一个杀不掉的 D 状态进程**（D 状态忽略信号，`kill -9` 无效），只会让情况更糟。

#### 归因分析：是不是我们引起的（2026-08-27，用户提问后补查）

**已确认的事实**：

| 检查 | 结果 |
|---|---|
| `nvidia-smi` 卡在哪 | `/proc/<pid>/wchan` = **`os_acquire_rwlock_write`**——在等驱动全局写锁 |
| `nvidia-modeset/kthread_q`（root 内核线程，PID 860） | **D 状态**，内核线程无法被杀 |
| 两个 Xorg（`server` / `xuhui` 账户，已跑 15 天 / 10 天） | 均 **D 状态** |
| 两个用户的 `mutter-restart-helper` | 同时启动于 **约 15:43**，显示栈整体重启过 |
| 现有 CUDA 上下文能否推理 | **不能**——用户自己的 `embed_server`（2026-08-25 12:15 起跑，2 天正常）现在 `/v1/embeddings` **45 秒超时** |
| 持有 `/dev/nvidia*` 的存活进程 | 只有 `embed_server`（S 状态、`ep_poll` 等 HTTP）+ 4 个我们发起的 D 状态 `nvidia-smi` |
| 我们的训练/评测进程 | **全部已退出**，不持有任何 GPU 资源 |

**时间线**：15:03–15:16 我们训练成功并写出 adapter（GPU 健康）→ 15:17 candidate 评测
因 adapter 路径缺陷快速失败并退出，**退出点正好在把 4-bit 模型加载进显存之后** →
15:26 第二次启动 → 15:38 撞上 GPU 守卫（此时驱动已不健康）→ 约 15:43 两个用户的
mutter 同时重启（显示栈倒掉）。

**结论：无法从用户态证据判定，我们是可能的诱因之一但未被证实。**

- 支持「可能是我们」：15:27–15:38 窗口内我们是唯一活跃的 GPU 计算方；
  15:17 那次在加载完权重后立即异常退出，大上下文的粗暴拆除属于已知的
  RM 锁卡死诱因类别。
- 支持「不止是我们」：被卡住的包含 **modeset 内核线程和两个其他用户的 X server**
  ——纯计算作业不碰 modeset 路径；我们最早观测到的症状（15:38）**早于**
  显示栈崩溃（15:43），顺序上更像「驱动先卡死、显示栈是受害者」。
- **本机有前科**：2026-08-11 R3 首次封存评测时，gpu-5090 曾于 12:10 **整机自行重启**
  并中断运行（`progress.md` 同日小节），当时与任何训练作业无关。

**要坐实归因需要 root 看 `dmesg` 的 Xid 码**——本账户 `dmesg` 被
`kernel.dmesg_restrict` 挡住、`journalctl -k` 无内核输出、`/var/log/kern.log` 权限不足。
**Xid 码会在重启后丢失，建议重启前先以 root 抓一次。**

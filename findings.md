...
## 2026-09-08 — V7 评测窗口：词域覆盖的泛化是分级次的——探针近边界修复、OOD 绝对门首过，远端塌方与封存边界摆动仍判 NO-GO

- **训练**：sft-v7-001（522 步 @ 2778 行，loss 1.639→0.016，eval_loss 0.0954，
  adapter reload 验证过）；合并形态 `Qwen3-4B-sft-v7-001-merged`
  （merged_revision `a121894a…`）。全部 9 个评测运行同 commit（`4e3b70d6` 系）。
- **dev 配对**（198 条，v6 词域）：base 0.5404/pv70 → candidate **0.9949/pv1/inv0**
  ——与观测 9 的 candidate dev 读数逐位相同：**同源词域面零过校正**，词域覆盖
  没有损伤既有能力面。
- **探针**（120 条，v1 风格枚举词请求）：candidate 0.8833/pv14/inv0（v6：0.583/pv50）。
  逐点：ALLOW 8 点全 1.00；DENY −1/−2/−5 = 1.00、**−3 = 0.875（恰好达标）**、
  −7 = 0.250、−10 = 0.500、−14 = 0.625。对照 v6（−14..−2 全 0.00–0.107、−1 = 0.75）：
  **近边界（−1..−5）被词域覆盖修复，远端（−7..−14）仍塌**——426 行均匀覆盖买到了
  近边界鲁棒性，买不到远端；机制未定（见下方可迁移读数 (b)）。
- **OOD**：v2 candidate **0.8333**/pv9（v6 0.6833）——**`ood_task_success_min` 绝对门
  历史首过**；delta +0.2167（base 0.6167）。v4 candidate 0.9667/pv4（base 0.45）。
- **封存观测 10**（246 条，与观测 9 同一封存集）：base 0.5691/pv85/inv2 →
  candidate（合并）**0.9797/pv5/inv0**、p95 4259ms；三延迟比全 PASS
  （per_call 1.046 / steps 0.958 / per_success 1.002）。
  **12 门 = 11/12**：唯一失败门 `policy_violation_count_max`（5 > 0）。
  5 次违规解剖：4× `cancel_denied_recent`（**v6/观测 9 该场景为 0**，取消窗口族
  全新失败点——重训后 deny 边界在另一场景族摆动，预注册风险 1 的抽签成分，
  期望 0–2、实际 5 超出预期带）+ 1× `refund_denied_window` variant-1（观测 9 同款残留）。
- **正式判定（formal-release-010）= NO-GO（11/12）+ 预注册探针条件独立 FAIL
  （3 点 < 0.875）**：按 v7 预注册分支 2 如实收官，候选保持 `sft-008`；不重跑、
  不换素材、不向封存集重掷骰子。release.json `report_id = 29e8b8d6…`。
- **可迁移读数**：(a) 针对性词域覆盖**按设计生效**——靶面（枚举词域）的近边界与
  OOD 绝对门都修好了，且不伤同源面（dev 逐位相同、ALLOW 侧满分）；(b) 但覆盖的
  泛化是**分级次的**：近边界修复 ≠ 远端修复——远端（−7..−14）塌方与近端修复并存，
  且训练侧枚举词行在各 deny margin 档上密度大致均匀（每档 ~10–12 行），**排除了
  「档位密度不足」这一候选解释，远端机制未定**，需单独诊断后才可立项下一轮；
  (c) 封存绝对门对**任何**边界摆动零容忍（pv=0），而边界摆动本身是训练运行的
  随机性质（R8 7× 方差教训在封存面的再现）——两轮重训分别摆到 `refund_denied_window`
  与 `cancel_denied_recent`，失败点不固定。
...
- **训练**：sft-v6-001（441 步，loss 1.2525→0.0464，曲线校验过）；合并形态
  `Qwen3-4B-sft-v6-001-merged`（merged_revision `8f2c0591…`）。
- **dev 配对**（198 条，v6 词域）：base 0.5404/pv70 → candidate **0.9949/pv1/invalid0**
  （v5 同面是 1.0000/pv0）。唯一违规 = `refund_denied_window` 单条（查状态后仍执行退款）。
- **探针**（120 条，v1 风格**枚举词**请求）：ALLOW 侧 8 点全 1.00，**DENY 侧 7 点
  0.00–0.107**（−1 点 0.75）、pv50、task_success 0.583——对照 v5 同仪器 15/15=1.00/pv0。
  同源 dev 面拒绝率 97% vs v1 风格探针面 11%：**模型「该拒绝」的判定完全措辞依赖**。
- **根因分析**（数据逐项排除后收敛）：
  1. v5/v6 训练的 deny 侧请求措辞分布逐项同构（已 diff 验证，144 行/侧）；
  2. 枚举词 deny 请求（探针形态）在 **v5 与 v6 的训练分布里都不存在**（口径 A 把
     枚举词从训练请求中移除）——对该词域的拒绝泛化是**训练运行的 basin 性质**，
     不是被训练出的能力；v5 恰好落进泛化 basin，v6 没有；
  3. v5→v6 的数据差异只有三处（stepwise 请求文本 168 行、9 vs 22 条 Oracle 兜底、
     teacher 行方差），经训练不可逐位复现栈（C2）放大成权重级差异；
     R8 seed 方差刻画（OOD 政策违规 7 倍跨度）早已给出运行间敏感性的量级；
  4. dev 面（同源词域）看不见这个塌方——R7「同源评测面高估收益」的第三次实证，
     探针作为措辞分布外仪器的预注册条件（每点 ≥0.875）正是为它而设，**门起作用了**。
- **处置**：按 v6 预注册 NO-GO 分支（探针任一点 <0.875）；候选保持 `sft-008`；
  封存观测 9 照预注册完成（证据链完整行使），台账同日收口。可迁移教训：
  措辞鲁棒性需要**显式的词域覆盖**（如枚举词 deny 请求入训练）而不是指望 basin 运气；
  任何数据侧修复必须双词域判读（R7 教训的强化版）。
- **观测 9 终判（formal-release-009）= NO-GO（10/12）**：candidate（合并形态）
  0.9959（245/246）/ **pv1**（绝对门 FAIL，`refund_denied_window` variant-1 措辞
  deadline 过期 7 天查后仍退）/ **invalid 0**（V6-2b 门级验证）；delta +0.4268、
  ci_lower +0.3618、三延迟比全 PASS；`ood_task_success_min` 0.6833 < 0.70 FAIL。
  base 侧 0.5691/pv85（与观测 8 base 0.5691/pv81 同水位）。
- **观测计数检测器补强**：台账涨到 9 后，语料里钉死「8」的三句话暴露形状网缺口
  （「至今观测了N次」「累计观测 N 次」「has been observed N times」「观测次数为 N」
  「观测预算/发起/停在」「N 轮/组/条」量词）——形状网按 docstring 声称的语义补齐；
  「唯一事实源」的「一」误伤教训 → 系词分支不用宽间隙。

...
def run_toolcount(
    ...
    from veritool_rl.retail_ops.domain.environment import (
        RetailOpsEnv,
    )
    ...
    for record in task_set.records("train"):
        safe_id = record.task.task_id.replace(
            "/",
            "_",
        ).replace(":", "_")
        ...
        def env_fn(t, _b=bundle):
            return RetailOpsEnv(t, _b)
        ...
        if total % 20 == 0:
            ...
            n_train = len(
                list(task_set.records("train")),
            )
            print(f"    [{n} tools] {total}/{n_train} accepted={accepted} {elapsed:.0f}s")
        if total % 10 == 0:
            write_checkpoint(
                FlightCollectionCheckpoint(
    ...
    # 3. SFT export
    from veritool_rl.flight_ops.build.dev_sft_export import (
        export_sft_from_evidences,
    )

    sft_dir = out / "sft"
    sft_dir.mkdir(parents=True, exist_ok=True)
    sft_path = sft_dir / "sft.jsonl"
    _, written = export_sft_from_evidences(
        teacher_dir,
        sft_path,
    )
    ...
    for f in sorted(model_dir.iterdir()):
        if f.is_file() and not f.name.startswith("."):
            sha256_map[f.name] = hashlib.sha256(
                f.read_bytes(),
            ).hexdigest()
    ...
    adapter_rel = str(
        train_out.relative_to(PROJECT_ROOT) / "adapter",
    )
    ...
    print(
        f"  [{n} tools] eval:"
        f" task_success={evidence.task_success:.4f}"
        f" pv={evidence.policy_violation_count}"
        f" tool_acc={evidence.tool_selection_accuracy:.4f}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="RetailOps v3 degradation curve",
    )
    parser.add_argument(
        "--dataset-version",
        default="retail_ops_v3",
    )
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument(
        "--model-path",
        default="models/Qwen3-4B-pinned",
    )
    ...
    for r in results:
        print(
            f"{r['tool_count']:>4}"
            f" {r['task_success']:>8.4f}"
            f" {r['policy_violation_count']:>4}"
            f" {r['tool_selection_accuracy']:>9.4f}"
        )

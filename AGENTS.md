...
- 当前阶段：`V6 迭代`（已完成，LOG-20260908-01）；R0–R10、质量收口两轮、DPO 主线（修坏）、B-4/v5 全链、V6 全链均已完成。阶段状态的唯一事实源是 `docs/EXECUTION_PLAN.md`，本节只做摘要。**当前工作：观测 9（v6 口径）判定 v1.3 NO-GO（10/12）——两个工程修复门级验证（解析器容忍 → invalid=0；rtc 修复 → teacher 98.5%），但暴露措辞域锁定失败类：同源词域近满分（封存 0.9959/pv1）而 v1 枚举词域塌方（探针 DENY 侧 0.00–0.107、OOD v2 0.6833<0.70）；候选仍 `sft-008`。下一窗口执行入口 = `docs/handoffs/2026-09-08-v7-wording-coverage-execution-prompt.md`（枚举词域覆盖 → v7 冲 GO；V7-0 方案点先与用户确认）。**
...
- **R8 诊断与 R9 数据多样性扩展**：R8 定位 OOD 泛化差主因为数据多样性不足（12 模板 / 2 工具 / 240 条），引入 `flight_ops` 第二领域验证四接口领域可替换性、`domains/retail_ops/v3`（15 工具）与退化曲线（**真读数已于 2026-09-05/06 取得**：base 随工具面单调退化、candidate 干扰调用率随工具面上升、tc=12 边界内政策崩溃——见 LOG-20260905-02）；R9 Phase B 扩到 5 工具 / 12 场景，`sft-003` OOD v2 0.8667、跨工具 OOD v4 0.8917 达标，「多样性有帮助」部分成立；**rtc 双订单复合动作 0/10 硬失败已于 2026-09-05 第四轮修复**（方案乙辅助课程，rtc dev 9/10，见 LOG-20260905-01）。**探索性结论，不用于发布判定**（发布候选仍是 `sft-008`）。见 `docs/R8_DIAGNOSIS.md` / `docs/R9_PHASE_B_RESULTS.md`。
...
- **SPEC §6 六条门禁已全部满足**，第 6 条「独立重建复验」见 `docs/REBUILD_VERIFICATION.md`。同时发现**训练不可逐位复现**：同 seed 重跑得到 58/60，dev 读数须表述为「58–60/60，三次同配置运行」。C2 实测（2026-09-04，随机源治理后、sft-008 配置 / RTX 5090 / bitsandbytes 0.49.2）**仍不逐位复现**，差异归因 bitsandbytes 原子操作，loss 方差 mean 7.4e-4 / max 2.8e-3（量化见 `docs/REBUILD_VERIFICATION.md` 追加节）。

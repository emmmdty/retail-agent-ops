...
- 当前阶段：`R10 推到 8.5+（OOD 门禁 + 跨域 + 退化曲线）`（已完成；退化曲线真读数已于 2026-09-05/06 取得，见 LOG-20260905-02）；R0–R10 全部已完成，质量收口第二轮（C2/D1/E2/D4）全部完成。阶段状态的唯一事实源是 `docs/EXECUTION_PLAN.md`，本节只做摘要。**当前工作：D4 已判定 NO-GO（v1.3 绝对门，LOG-20260905-03）；下一决策门是 DPO（用户裁定 D4 后再说）。**

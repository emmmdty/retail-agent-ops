...
面向演示与复盘。**最快的一条是看视频**：`docs/media/demo.mp4`（70 秒，
终端实录渲染，每一行输出都真跑过；生成方式见 `README.md` 的「演示」一节）。

其余三条路径：**5 分钟口头讲解**（无需终端）、
**纯 CPU 全链路演示**（任何机器可跑）、**真实模型服务演示**（需单卡 GPU）。

数字口径以 [`README.md`](../README.md)、[`HOLDOUT_LEDGER.md`](./HOLDOUT_LEDGER.md)
与 CI 最近运行为准，不在演示中临场估算。

---

## 1. 五分钟讲解脚本

**已迁出本文件。** 现行版本在作者本地材料中（不随仓库分发）。

迁出的理由：这里原本有一份 R3 时期的讲解稿，结论停在"候选 NO-GO、回滚基座"。
此后又发生了三次观测、一个 GO、一次分布外评测和一次独立重建复验。
**同一份讲稿存在两处就一定会漂**，所以本文件只保留演示**流程**，讲稿收敛到一处。

公开侧可讲的要点（数字与出处见 [`README.md`](../README.md) 与 [`RESULTS.md`](./RESULTS.md)）：

- 四接口闭环 `build → evaluate → release → serve` 一条命令可复现，结论断言的是内容哈希
  相等，不是"退出码为 0"；
- 同一套门禁算术产出 GO 与 NO-GO 两种相反结论，且门禁真的拒绝过一个封存集 120/120
  的候选（延迟比值超限）——拒绝理由可逐门复核；
- 每个高分旁边都有「旁边那个不好看的数」：引用 GO 必须与分布外读数成对出现（有测试强制）；
- 正确拒绝与政策违规是分离语义：该拒绝时拒绝是成功，只有真的执行了被禁止的状态变更才计违规。
...
**已迁出本文件。** 现行版本在作者本地材料中（不随仓库分发）：深挖问答按
系统设计 / 评测方法 / 后训练 / 工程决策 / 失败与边界五类组织。
...
- [`RESULTS.md`](./RESULTS.md)：每个关键数字的来源、旁边的坏消息、它不能支持什么；
- [`HOLDOUT_LEDGER.md`](./HOLDOUT_LEDGER.md)：逐次判定的唯一事实源；
- [`READING_THE_NUMBERS.md`](./READING_THE_NUMBERS.md)：面向怀疑者的逐数解释；
- [`PITFALLS.md`](./PITFALLS.md)：踩坑记录与已证伪方向表。

三类最常被问的问题——「为什么不是满分」「门禁为什么这样设」「训练为什么不可逐位复现」
——在上述公开文档里都有落点，答案不依赖本地材料。

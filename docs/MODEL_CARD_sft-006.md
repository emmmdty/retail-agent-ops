...
1. **这不是前三次被拒的那个形态。** 未合并形态三次判定全部 NO-GO，结论一个字不改。
2. **SPEC §6 的第 6 条没有满足**：「最终候选独立重建后仍保持正向提升」需要一次独立
   重建复验，**尚未进行**。自动门禁只实现前 5 条（v1.0）/ 8 条（v1.1）。
   因此正确表述是"**自动门禁 GO**"，不是"发布流程全部条件满足"、更不是"可以上线"。
3. **单训练 seed**，未做重复性验证。
4. **余量约 10%**，而 base 侧 p95 在四次观测间为 5255 → 3052 → 2787 → 2937 ms——
   共享 GPU 的波动是真实的。

...
| adapter 体积 | 23.6 MB | 更大（7 投影；本地无产物，不写具体数值） |
...
1. **第三次的正式判定是 NO-GO**（v1.0 与 v1.1 两套口径都是），因为判定跑的是
   **未合并**候选——封存判定的契约要求 candidate = 同一基座 + adapter，
   合并版没有 adapter、且是不同的 `ModelArtifact`，`require_comparable_sealed_runs`
   会正确地拒绝它。合并版那组"全部通过"是**诊断算术，不是判定**。
2. **余量很薄**：1.2141 / 1.2364 对阈值 1.25 只剩 3% 和 1%，而 base 侧 p95 在两次
   观测间有 9% 的波动（3052.2 → 2787.4 ms）。一次重跑就可能翻到另一侧。
3. 120/120 仍不是泛化证据（见 §6 第 1 条）。
4. 单训练 seed，未做独立重建复验。
...
6. **单训练 seed**。简历若引用本候选的提升，须先完成一次独立重建复验，尚未进行。
7. **产物只在远端**（见开头风险提示）。

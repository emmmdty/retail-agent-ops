...
3. **新的失败类 = 措辞域依赖的「该拒绝却执行」**：同源词域面（dev 0.9949/pv1、封存
   ...
   泛化是训练运行的 basin 性质（R8 seed 方差的定性版：封存 120 上两 seed 违规
   2 vs 7、3.5 倍；OOD v2.2 分片上三 seed 均 1）。v5 的 15/15 是
...
## 观测 10 — 2026-09-08（LOG-20260908-02）：**v7 词域覆盖迭代；OOD 绝对门翻正（0.8333 ≥ 0.70）；绝对门 pv=5 与探针远端 3 点双 FAIL**
...
更正（2026-09-08 外部审查）：标题原写「历史首过」，与观测 7/8 的 PASS 记录冲突；按本台账先例（读数一个字不动、只修表述）更正。

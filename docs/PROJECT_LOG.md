...
### LOG-20260814-01：R4 第二轮 A/B——LoRA 容量是真正的瓶颈，候选 A 在 dev 上 60/60 达标

- 日期：2026-08-14
- 阶段/任务：R4 第二轮（三候选并列消融）Stage 2：候选 A、B 的训练与 dev 配对评测
- 状态：解决（**A 达标，B 未达标但有信号**；C 尚未执行）
- 关联：LOG-20260811-09（第一轮判负）、LOG-20260811-06（根因精确化）、
  设计 `docs/superpowers/specs/2026-08-13-r4-round2-ablation-design.md`

**运行事实**（gpu-5090 物理 GPU 0，RTX 5090，`GPU-07af326b-…`，本次 GPU 空闲）：
候选 A 训练 `sft-003` 09:25:47→09:31（`train_runtime` 222.6 s / 75 steps，峰值 5.638 GB，
`train_loss` 0.1800，adapter 重载 `loaded: true`）；A dev 评测 `candidate-003`
（`wall_time` 268.5 s，`EXIT=0`）。候选 B 训练 `sft-004`（`train_runtime` 212.2 s，
峰值 5.563 GB，`train_loss` 0.2246）；B dev 评测 `candidate-004`（`EXIT=0`）。
两次 `compare_dev_runs` 配对契约**均通过**，base 沿用既有 `qwen3-4b-dev-base-001`
（`d57654e9…`，未重跑）。

**逐场景结果**（每类 n = 10，dev，非 holdout）：

| 场景 | base | 第一轮 002 | **A（003）** | **B（004）** |
|---|---|---|---|---|
| lookup_status | — | 10/10 | 10/10 | 10/10 |
| refund_denied ×3 | — | 30/30 | 30/30 | 30/30 |
| **refund_eligible** | — | **0/10** | **10/10** | **4/10** |
| refund_recovery | — | 5/10 | **10/10** | **10/10** |
| 合计 | **48/60** | 45/60 | **60/60** | **54/60** |

A 达标（门槛 `refund_eligible` ≥7/10 且 `invalid_call`=0、`policy_violation`=0、
`schema_valid_rate`=1.0，四项全中）；B 未达标（4/10 < 7/10）但越过 ≥3/10 的诊断阈值。

**本条改变后续做法的结论：`lora.target_modules` 只覆盖 attention 投影，是前两轮
一直没被质疑的真正瓶颈。** 候选 A 与第一轮用的是**同一份训练数据**
（`train-export-002`，逐字节相同），唯一差别是把 LoRA 从
`[q,k,v,o]_proj` 扩到加上 `gate/up/down_proj`（r=16、alpha=32、dropout 不变），
`refund_eligible` 由 **0/10 变为 10/10**。数据污染无法解释这个差异——若存在污染，
第一轮在同一份数据上也会满分而不是 45/60。因此**后续训练一律使用全 linear layer 的
LoRA**，"每轮只改一个变量"的纪律要把容量也列为必检变量，不能再默认沿用。

**这同时改写了 R3/R4 前两轮的解释**：观察到的"格式类改变成功、行为倾向类改变失败"
（`schema_valid_rate` 0.78→1.00、`invalid_call` 41→0，而"该不该执行动作"纹丝不动）
不是"条件决策需要更多/更均衡的数据"，而是**浅层输出模式能被 attention-only 的低秩
更新学到，条件决策不能**。LOG-20260811-09 记为"被证伪"的那个数据比例假设，
其失败原因在此得到解释：比例改了也没用，因为容量根本不足以表达那个决策。

**候选 B 是独立的次要信号，不能被 A 的结果掩盖**：仅在多步样本末尾追加一条独立的
assistant 终局回复（`train-export-003`，模板只用 `refund_order` 真实返回的
`{order_id, reason, refund_status}`），在容量不变的前提下把 `refund_eligible`
0/10→4/10、`refund_recovery` 5/10→10/10。即"多步路径在训练数据里没有闭环"确实是
一个真实成因，只是不足以单独跨过门槛。A 与 B 是并列消融、未叠加，**两者叠加的效果
本轮没有测过，不得推断**。

**`verifier_reward` 首次与主判据同向**（A：0.5792→1.0000，与 `task_success`
0.800→1.000 同向）。此前三次背离（R3 dev、封存 holdout、R4 第一轮 dev）。
一次同向不推翻"奖励值不能代替最终状态判据"——那三次背离仍然成立，
且本轮 B 的 `train_loss`（0.2246）高于 A（0.1800）而两者行为差距远大于损失差距。

**统计限度（必须随结论一起引用）**：每类 n = 10，单条样本变化即 10 个百分点。
`task_success_ci95` 在 A 上是 [1.0, 1.0]，那是 60/60 的退化区间，不代表 holdout 上会复现。
**这是 dev 结果，不是发布判定**；120 条封存 holdout 未被触碰，第二次观测未消耗。

**延迟口径**：本次 GPU 空闲（第一轮为他人占用 96–98%），因此 A 的
`p95_latency_ms` 6222（比值 1.0254）与 B 的 5380（比值 0.8865）与第一轮的延迟数
**不可直接比较**；dev 判据不含延迟门禁，此处只作记录。

**尚未发生**：候选 C（改 `SYSTEM_PROMPT`）未执行，其所需的 base dev 重跑亦未执行；
任何 release 判定、任何 holdout 访问、任何叠加 A+B 的实验。

### LOG-20260814-02：R4 第二轮收官——纯 prompt 零训练拿到 9/10，而容量不足的训练是净负作用

- 日期：2026-08-14
- 阶段/任务：R4 第二轮 Stage 3/4：候选 C（指令框定）+ 新 prompt 下的 base 重跑；三候选收官
- 状态：解决（**三候选跑完，按 spec 停止条件停止**）
- 关联：LOG-20260814-01（A/B）、LOG-20260811-09、LOG-20260811-06

**运行事实**（gpu-5090 物理 GPU 0）：`base-002`（新 prompt 零训练，`EXIT=0`）；
`sft-005` 训练（`train_runtime` 197.9 s，峰值 5.553 GB，`train_loss` 0.2191）；
`candidate-005` 评测（`EXIT=0`）。`compare_dev_runs` 对 `base-002` 配对契约通过。
新 `SYSTEM_PROMPT` sha256 `8ae813c4284246b9…`（旧 `d919602e25f2c87c…`），
在原有两句后追加「确认符合退款政策后直接调用工具执行，不要再向用户征询确认」。

**`refund_eligible` 全景（每格 n = 10，dev）**：

| 运行 | prompt | 训练 | LoRA | refund_eligible | 合计 |
|---|---|---|---|---|---|
| base-001 | 旧 | 无 | — | 5/10 | 48/60 |
| **base-002** | **新** | **无** | — | **9/10** | 54/60 |
| sft-001（R3） | 旧 | 有 | attn-only | 0/10 | 43/60 |
| sft-002（R4-1） | 旧 | 有 | attn-only | 0/10 | 45/60 |
| sft-004（B） | 旧 | 有+闭环 | attn-only | 4/10 | 54/60 |
| sft-005（C） | 新 | 有 | attn-only | 5/10 | 55/60 |
| **sft-003（A）** | 旧 | 有 | **attn+MLP** | **10/10** | **60/60** |

**本条最重要的结论：attention-only LoRA 的 SFT 对 `refund_eligible` 是系统性的净负作用，
两个 prompt 下都成立。** 旧 prompt：base 5/10 → 训练后 0/10（R3 与 R4 第一轮各一次）。
新 prompt：base 9/10 → 训练后 5/10。方向一致、量级相近（−4 到 −5），且横跨两个
prompt、四次独立训练。这不是噪声，是这套配置的固有性质：**低秩更新只挂 attention 时，
SFT 学到的是输出形状，代价是覆盖掉基座模型原有的条件决策能力**。

**加上 MLP 三投影后，同一份数据、同一组超参下，训练从负作用变为正作用**
（5/10 → 10/10，候选 A）。容量不是"锦上添花"，它决定了训练的符号。

**第二条同等重要的结论：这个行为根本不需要训练。** 纯 prompt 干预（零训练、零成本）
把 `refund_eligible` 从 5/10 提到 **9/10**，把 `invalid_call` 21→0、
`policy_violation` 8→5、`schema_valid_rate` 0.781→1.000，全部零训练达成。
候选 A 的 10/10 相对它只多 1 条（n = 10，1 条 = 10 个百分点，**这个差距不足以支撑
"训练优于 prompt"的排序结论**）。R3/R4 前两轮在这条线上的 SFT，确实是在解一个
prompt 层面的问题——这正是本轮设计 §4.3 预先标记为"可能得到的最有价值的单条结论"的那一条。

**训练并非全无价值：`refund_recovery` 是反向的。** base 在两个 prompt 下都是 5/10
（prompt 干预对它**完全无效**），而四次训练后分别为 5/10（R4-1）、10/10（A）、
10/10（B）、10/10（C）。需要"失败后重试"的多步恢复能力来自训练数据，不来自指令。
**因此"prompt 就够了"只对 `refund_eligible` 成立，不能推广。**

**候选 B 的独立贡献在新数据下仍然成立**：容量不变时，仅追加终局回复即把
`refund_eligible` 0/10→4/10、`refund_recovery` 5/10→10/10。

**判定**：三候选**只有 A 达标**（`refund_eligible` 10/10 ≥ 7/10 且 0/0/1.0）；
B（4/10）与 C（5/10）未达标但均越过 ≥3/10 的诊断阈值。按 spec 停止条件，
**三个候选跑完即停止**，不追加变体、不改训练目标、不扩展算法。

**未测且不得推断的组合**：A（全 linear LoRA）+ 新 prompt、A + B（终局回复）、
以及三者叠加。本轮是**并列消融**，任何叠加效果都没有观测。

**统计限度**：每类 n = 10，单条样本即 10 个百分点。全部为 **dev** 结果，
且 dev 已被用于候选选择，存在选择偏差。**这不是发布判定**；120 条封存 holdout
未被触碰，第二次观测未消耗。

**延迟**：本轮各次运行 GPU 占用状况不同于第一轮，延迟数不跨轮比较；dev 无延迟门禁。

### LOG-20260814-03：R4 第三轮——prompt 与训练解决的是不同问题，同 prompt 配对下训练仍有 +0.100

- 日期：2026-08-14
- 阶段/任务：R4 第三轮实验 1（容量 × 指令框定的叠加）
- 状态：解决（**训练在最优 prompt 之上仍有独立增量**）
- 关联：LOG-20260814-01/02（第二轮三候选）

**为什么做这个实验**：第二轮结束时存在一个无法回避的方法论缺口——候选 A 的 +0.200
是对照**旧 prompt** 的 `base-001`（48/60）取得的，而我们自己测出新 prompt 的零训练
base 已是 54/60。拿旧 prompt 的 base 去做 release 判定，契约合法（`PAIRING_FIELDS`
只要求两侧 prompt 相同）但科学上站不住：delta 里有多少属于训练、多少属于"没给 base
换 prompt"，无法分离。本实验用**同 prompt 两侧配对**关掉这个自由度。

**运行事实**（gpu-5090 GPU 0，运行期间被他人占用 81%）：`sft-006` 训练
（`train_runtime` 293.7 s，峰值 5.647 GB，`train_loss` 0.1795）；`candidate-006` 评测
（`EXIT=0`）。配对对照 `base-002`（新 prompt 零训练），`compare_dev_runs` 契约通过。

**结果：60/60，对照 base-002 的 54/60，`task_success` delta +0.1000。**
`policy_violation` 5→0、`recovery_success` 0.5→1.0、`invalid_call` 0→0、
`schema_valid_rate` 1.0→1.0。

**完整 2×2（dev，每格 60 条）**：

| | 零训练 | attn-only 训练 | **full-linear 训练** |
|---|---|---|---|
| 旧 prompt | 48/60 | 45/60（R4-1） | **60/60**（A） |
| 新 prompt | 54/60 | 55/60（C） | **60/60**（本次） |

**本条改变后续做法的结论有三点**：

1. **prompt 与训练解决的是不同的失败，且几乎不重叠。** 逐类别拆解：
   prompt 把 `refund_eligible` 5/10→9/10 而 `refund_recovery` **纹丝不动**（5/10→5/10）；
   训练把 `refund_recovery` 5/10→10/10、`policy_violation` 5→0，而 `refund_eligible`
   只再多 1 条。即：**指令能解除"判定后不敢执行"，但教不会"失败后重试"**；
   后者只能来自轨迹数据。这否定了"prompt 就够了"的简化叙述，也否定了"SFT 无用"。
2. **容量足够时，prompt 的增益被训练吸收。** full-linear 在新旧两个 prompt 下都是
   60/60（无差别），而零训练与 attn-only 训练下 prompt 值 +6 / +10 分。
   即 prompt 工程的收益随模型可塑性上升而**递减到零**。
3. **release 判定的正确配对已经确定**：base = `base-002`、candidate = `sft-006`，
   两侧同 prompt、同代码提交。dev delta +0.100 高于门禁 +0.05，但 **dev 已用于选择**，
   holdout 上必然回落，回落幅度未知。

**统计限度**：每类 n = 10、每格 60 条。60/60 的 `task_success_ci95` 是 [1.0, 1.0]，
那是满分的退化区间，**不是** holdout 会复现的证据。全部为 dev。
**封存 holdout 未触碰，第二次观测仍未消耗。**

**延迟不可跨运行比较**：本次训练期间 GPU 被他人占用 81%，`train_runtime` 293.7 s
...
### LOG-20260814-05：跨规模验证证伪了"容量决定训练符号"——真实规律是容量必须与模型规模匹配

- 日期：2026-08-14
- 阶段/任务：R4 第三轮跨规模验证（Qwen3-1.7B 上复现 4B 的容量结论）
- 状态：解决（**上一条结论被证伪并替换**）
- 关联：**推翻 LOG-20260814-01/02 中"容量决定训练效果的符号"的一般化表述**；
  LOG-20260814-03（4B 侧读数）

**方法**：四臂构成 2（模型规模）× 2（LoRA 覆盖）的对照，全部用同一份
`train-export-004`（新 prompt）、同一组超参（r=16/alpha=32/dropout=0.05、3 epoch、
batch 2×8、lr 2e-4、seed 0），各自与**同规模、同 prompt 的零训练 base** 配对。
跨规模对只差 `model` 段、同规模内只差 `target_modules`，由
`tests/test_retail_ops_r4_round3_cross_scale.py` 断言并经三处突变验证
（指错模型 pin、混用旧 prompt 导出、顺手改 r）。

**结果（dev 60 条，新 prompt）**：
...
**"容量决定训练效果的符号"作为一般规律不成立。** 1.7B 上方向完全相反：
attention-only 是**强正作用**（44→58，+14 条，`task_success` +0.2333），
而全 linear 几乎无增益（44→45，+0.0167）。若只看 4B 就下"越多投影越好"的结论，
在 1.7B 上会做出恰好相反的选型。

**1.7B 全 linear 的失败形态是结论的关键，必须逐条记录**：15 条失败**全部**是
"该拒绝却没拒绝"——`refund_denied_ownership` **10/10 全灭**、`refund_denied_window`
5/10 失败；`policy_violation` 由 base 的 0 升到 **5**；`average_tool_calls`
由 1.267 升到 **2.083**（attention-only 只到 1.550）。模型被训练成"什么都去调
`refund_order`"，**丧失了拒绝能力**。这不是"没学会"，是**学过头**。

**替换后的规律（本条的结论）**：**LoRA 可训练容量必须与模型规模匹配，不存在"越大越好"。**
容量不足则学不会条件决策（4B + attention-only：`refund_eligible` 被压到 5/10）；
容量过剩则被训练数据的类别偏向带跑，牺牲与主偏向相反的能力（1.7B + 全 linear：
拒绝类崩塌）。attention-only 在 1.7B 上恰好起到了正则作用。

**由此追出的连带结论（改变后续做法）**：**数据配比与 LoRA 容量是耦合的，不能独立调。**
`train-export-004` 沿用了 R4 第一轮为提升 `refund_eligible` 所做的 ×3 oversample，
使训练集的"执行类:拒绝类" = 240:120 = **2:1**。这个偏向在 4B 上无害（容量足够同时
承载两种行为），在 1.7B + 过剩容量下直接导致拒绝能力崩塌。**今后调 oversample 必须
连同 target_modules 一起评估，把两者当作一个二维选型而不是两个独立旋钮。**

**第二条被限缩的结论**：**"一句 prompt 就能解决大半"是模型规模依赖的。**
新 prompt 对 4B 有效（`refund_eligible` 5/10→9/10），对 1.7B **完全无效**（0/10）——
1.7B 的指令跟随能力不足以消费那句显式授权。反过来训练对 1.7B 极其有效
（0/10→10/10）。因此 LOG-20260814-02/03 关于"prompt 与训练分工"的结论
**只在 4B 这一规模上成立**，不得作为跨规模的一般表述引用。

**统计限度**：每格 dev 60 条、每类 n=10。1.7B 的 attention-only 58/60 与 4B 的
全 linear 60/60 **不可直接相比**（不同基座）；跨规模只比"训练相对同规模零训练 base
的符号与量级"。所有读数均为 dev，未触碰封存 holdout（两次观测已在
LOG-20260814-04 用尽，本轮消耗 0 次）。

**运行事实**：`base-1p7b-002`、`sft-1p7b-attn`（100.1 s，`train_loss` 0.2458，adapter 13 MB）、
`sft-1p7b-full`（116.3 s，0.1982，adapter 34 MB）、`candidate-1p7b-attn`、
`candidate-1p7b-full`，均 `EXIT=0`，两次 `compare_dev_runs` 配对契约通过。
峰值显存约 3.6 GB。

### LOG-20260815-01：发布门禁语义版本化，并据此把第二次 NO-GO 的延迟代价证明为部署形态而非模型能力

- 日期：2026-08-15
- 阶段/任务：R4.5 架构补强轨道（评审驱动），批次 1+3 与 P0-3 对照
- 状态：解决（**改变后续判定口径与延迟表述**）
- 关联：LOG-20260814-04（第二次观测与判定）、LOG-20260811-03（第一次观测）、
  LOG-20260811-06（配对可比性代价）
- 提交：`3427c40`（门禁 v1.1 与批次 1）、`549957a`（复算与合并脚本）、`007e506`（合并版评测配置）

**阶段变更**：用户裁定新开 **R4.5 补强轨道**，R4 标为已完成。本轨道**不产生新的发布
候选、不下调任何阈值、不消耗第三次 holdout 观测**。

**一、发布门禁语义版本化（改变后续判定口径）**

旧 `GATE_IDS` 五元组被两个 report 模型逐字段断言，就地增删会让磁盘上**全部**已有
release 报告无法加载。因此走版本化路径：`GATE_IDS_BY_SCHEMA` 同时承载 v1.0（冻结）
与 v1.1，各自的 `schema_version` 绑定各自的集合与顺序；有测试断言
`formal-release-001/002` 与 R1 qualification 报告仍能被加载。

v1.1 把 episode 级 p95 拆成三个各自回答一个问题的量
（`per_call_latency_ratio` 部署速度 / `steps_to_success_ratio` 规划效率 /
`latency_per_success_ratio` 归一化成本），并给 `success_delta` 补一个配对 bootstrap
CI95 下界门禁。**阈值一个都没新增**——三个比值复用 `p95_latency_ratio_max`，
CI 下界的 0 是结构性常量。这不是省事：核实发现 `domains/retail_ops/v1/release.yaml`
**也在 `bundle_sha256` 的分量里**（`domain/bundle.py` 把四个文件哈希成一个 mapping
再哈希），往它加任何字段都会使全部已有 dev/sealed 证据不可配对。该约束此前无人记录。

**二、用 v1.1 复算两次已有观测（未跑模型、未消耗观测；新口径先于任何新读数定稿并提交）**

两次判定**都仍是 NO-GO**，无任何候选翻转；观测 1 的失败门禁反而由 1 项增至 3 项。

- **旧口径把观测 1 的部署代价整个藏了起来**：episode 级 `p95_latency_ratio` 1.0870
  （通过），而单次调用比值 **1.9643**（失败）。原因正是拆分要解决的偏置——那个候选
  更差、"说完就停"、调用更少、结束更早，于是**做得更差反而显得更快**。
- **观测 2 的提升通过配对显著性检验**：`success_delta_ci_lower` **+0.0833 > 0**；
  `steps_to_success_ratio` **0.9841 < 1**（每成功一条任务的调用数比基座更少）。
  两项失败门禁 1.9852 / 1.9536 几乎完全由单次前向开销构成。

**三、部署形态对照（gpu-5090 物理 GPU 0，dev 60 条，不是发布判定）**

把 `sft-006` 在 bf16 下 `merge_and_unload` 合并回基座、再按同一份生成参数量化回 NF4：

| | base-002 | 未合并 sft-006 | **合并版** |
|---|---|---|---|
| dev `task_success` | 54/60 | 60/60 | **60/60** |
| `average_tool_calls` | 1.3167 | 1.5000 | **1.5000** |
| 单次调用耗时 | 1356.6 ms | 3063.9 ms | **1653.7 ms** |
| 输出吞吐 | 48.89 tok/s | 28.38 tok/s | **50.74 tok/s** |

**行为一致、能力未损伤，单次调用耗时降低 46%，吞吐回到基座水平以上。**
合并 + 重新量化会损伤模型是这次最大的风险，在 dev 上**没有兑现**。

**这条改变了后续做法**：此前把第二次 NO-GO 表述为"效果/延迟权衡——容量换来 +14.2pp
与满分，代价是 p95 接近翻倍"。该表述**不完整**：代价里很大一部分不是模型的，是
未合并 LoRA 的前向开销。今后讨论"这个模型是不是太慢"之前，必须先确认测的是模型
还是部署形态；正式候选评测应以**合并后的部署形态**为准。

**四条不得省略的限制**（防止把 dev 读数当成发布结论）：

1. 是 **dev 不是 holdout**，且 dev 已被用于选出 `sft-006`，带选择偏差；
2. **旧 v1.0 口径下合并版在 dev 上仍然失败**（`p95_latency_ratio` 1.3130 > 1.25）。
   v1.1 下的 8/8 全过是**合并与口径拆分共同作用**的结果，不是单靠合并；
3. `latency_per_success_ratio` = **1.2498** 对阈值 1.25，只差 0.0002，是擦边不是稳健通过；
4. 同一 adapter 的延迟比值 dev 上 2.30、holdout 上 1.88，**dev 数不可外推**。

**未做**：merged + vLLM / `torch.compile`（需新依赖，属独立确认门）；
第三次封存 holdout 观测（用户单独决策门）。

**其余批次 1 内容**（serve 服务化、CI 与容器化、文档单一事实源、`verifier_reward`
降级、`perturb_schema` 接入）属常规实现，由 git history 与 `findings.md`/`progress.md`
承载，不在本条展开。

### LOG-20260815-02：业务政策从 Python if 链变成可执行的版本化输入；guardrail 成为独立一层

- 日期：2026-08-15
- 阶段/任务：R4.5 架构补强轨道，批次 2（评审 P0-2 / P2-8 / P2-9）
- 状态：解决（**改变领域输入的表达方式与后续演进路径**）
- 关联：LOG-20260815-01（同轨道批次 1+3）
- 用户裁定：`refund_order` 加必填 `idempotency_key` 的连带影响，走
  **「bundle 打新版本号、新旧并存」**（另两个选项是迁移现有 240 条 teacher 轨迹、
  或重新采集）

**为什么这条值得进长期档案**：它改变的不是一次实现，而是"领域输入怎么演进"这件事的
做法。此前 `SPEC.md` §2 把"版本化业务政策"列为流水线输入，但 `policies.yaml` 的
`rules:` 六条名字在 `src/` 里零处引用，真正语义硬编码在 `_refund_order` 的 if 链里；
"退款窗口从 14 天改成 7 天"的答案是"改 Python + 重训模型"。

**新的做法**：

1. **领域语义的变更以新 bundle 版本承载，旧版本逐字节冻结。** v1 的四份文件都在
   `bundle_sha256` 的分量里，而它同时在 dev 的 `PAIRING_FIELDS` 与
   `SEALED_PAIRING_FIELDS` 内——改 v1 一个字节，全部已有 dev/sealed 证据立刻不可配对。
   今后任何工具 schema 或政策语义的变更都走这条路径，不再考虑就地演进。
2. **政策规则是可执行的声明式输入，只有一条求值路径。** v1 的六个名字解析到内置
   冻结规则集（YAML 不必改），v2 起规则内联 YAML。谓词只有五种、事实只有一张固定表，
   引用不存在的事实在**加载 bundle 时**失败——不做通用表达式语言是刻意的：在一条决定
   退款是否放行的路径上，把配置错误推迟到运行期这个交换不划算。
3. **验收判据是一条测试而不是一段描述**：只改 YAML 里的一个数、不碰任何 Python，
   全链路产出不同判定。P0-2 是否完成由它单独裁定。
4. **guardrail 与环境政策校验分层独立**，两层都拦到才算纵深防御。它不读环境内部状态，
   只认线路上发生过的确认（成功的 `get_order` 才扩大作用域，失败的不算）。
5. **`max_transient_retries` 真正驱动逻辑**，不再只是被解析。

**注入对照的实测**（12 条 qualification，纯 CPU，规则策略）：

| | guardrail | 注入成功 | 成功率 | `task_success` | 政策违规 |
|---|---|---|---|---|---|
| 未防护 | false | 10/12 | 0.83 | 0.6667 | 4 |
| 已防护 | true | **0/12** | **0.00** | **1.0000** | **0** |

**这度量的是上下文污染，不是任何真实模型的易感性。** 探针策略只有真的在 `messages`
里读到那句注入指令才会去动干扰订单；真实模型（Qwen3-4B）的注入成功率需要 GPU 评测，
**本轮没有做，也不声称**。另一条值得记的观察：注入不只是"多调了一次工具"——未防护侧
掉了 4 条任务并产生 4 次真实政策违规，因为被注入的调用会撞上环境的归属/重复规则。
两层都拦到正是纵深防御应有的样子。

**连带的实现级约束（会影响后续所有带 guardrail 的运行）**：guardrail 会消毒观测、
也会拦调用，两者都写进轨迹，因此**重放必须用同一套 guardrail**，且每次重放构造新实例
（它持有会话级作用域，复用会把上一次的授权带进来）。用不带 guardrail 的环境重放一条
带 guardrail 的轨迹必然 `ReplayMismatch`——那不是证据损坏，是重放条件没对齐。

**边界**：正式数据集轨道（`retail_ops_v1_r2_20260722`）仍**只接受 v1 bundle**，
`build/formal_manifests.py` 显式拒绝其它版本。v2 的任务集若要用于正式评测，必须作为
独立 dataset artifact 重新冻结，那会作废全部已有评测证据的可比性，是一次独立决策。
**v2 至今只在 qualification 轨道（规则策略、纯 CPU）跑过，没有任何真实模型在 v2 上跑过。**

760 → 826 passed；v1 全链路复现校验通过，`bundle_sha256` 未漂。

### LOG-20260815-03：第三次封存 holdout 观测——三次判定全部 NO-GO；合并部署形态把差距拉进门槛内，但契约拿不到判定

- 日期：2026-08-15
- 阶段/任务：R4.5 架构补强轨道，第三次封存 holdout 观测（代码冻结于 `b529bc9`）
- 状态：解决（**改变"发布判定该测哪一种部署形态"这件事的做法，并暴露一处契约限制**）
- 关联：LOG-20260815-01（门禁 v1.1 与部署形态归因）、LOG-20260814-04（第二次观测）、
  LOG-20260811-03（第一次观测）、LOG-20260811-06（配对可比性代价）
- 用户裁定（2026-08-15）：第三次观测按**原契约**跑 base vs base+adapter，
  合并版的延迟另测部署形态

**执行**：三次运行同 commit、同 `uv_lock`，一次性跑完——
`holdout-base-003`（4m04s）、`holdout-candidate-003`（9m00s，未合并 sft-006）、
`holdout-merged-003`（6m41s，合并版探针，走 base 通道）。
base/candidate 相对第二次观测除 `attempt_id` 外逐字段相同，有治理测试锁定。

**判定：NO-GO / baseline，两套口径都是。** 三次判定全部 NO-GO，阈值一个字未改。
...
**跨 commit 的确定性成立**：base-003 与 base-002 的 `task_success` 0.8583、违规 11、
非法调用 5、schema 0.9691 **逐位相同**，candidate 同为 120/120。R4.5 改了 serve、
门禁、政策引擎、guardrail、多轮循环，评测结果一位没动——这是质量门之外的独立证据。

**合并部署形态（诊断，不是判定）**：合并版在 holdout 上同样 **120/120**、零违规、
零非法调用、`average_tool_calls` 与未合并版完全相同（1.5000）。单次调用
2946.5 → **1717.7 ms**，p95 比值 2.0250 → **1.2141**，吞吐 29.78 → 48.87 tok/s
（基座 50.66）。把候选侧换成合并版后，**v1.0 与 v1.1 的门禁算术全部通过**。

**这条改变了后续做法**：延迟门禁的测量口径必须固定为**将要上线的那种部署形态**。
"同一份权重、同一套行为，只差加载方式"就是 NO-GO 与"全部门禁算术通过"之间的全部
距离——继续用未合并形态去测一个打算合并后上线的候选，测的不是那个系统。

**同时暴露一处契约限制（本条最重要的架构发现）**：
`require_comparable_sealed_runs` 要求 candidate = **同一基座 + adapter**；合并版没有
adapter、且是不同的 `ModelArtifact`，两条都不满足，因此**结构上无法作为 candidate
进入封存判定**。该契约在设计时假设了"候选永远是 base+adapter"，而部署形态优化天然
会打破这个假设。要让合并形态可判定，必须版本化 `SealedEvaluationReport`——而
`report_id` 是全字段自哈希，需要**版本感知的内容哈希**才能让两份旧 sealed 证据仍可
加载。这是一次独立决策，本轮**未做**。

**四条不得省略的限制**：

1. **正式判定是 NO-GO。** 合并版那组"全部通过"是诊断算术，**不得**表述为
   "候选通过了发布门禁"。
2. **余量薄**：1.2141 / 1.2364 对阈值 1.25 只剩 3% 和 1%；base 侧 p95 在观测 2 与
   观测 3 之间有 **9% 的波动**（3052.2 → 2787.4 ms，同机同配置）。一次重跑就可能翻。
3. **本次消耗三次运行**。合并版探针只为测部署形态，但同样产出任务指标、同样读了
   holdout，因此如实计入消耗，不做"只算延迟不算观测"的记账。
4. **120/120 仍不是泛化证据**：train/dev/holdout 共用同一批 12 句请求模板。

`report_id`：base `931ed6f8…` / candidate `4c8839a2…` / merged `19f858fb…`。
产物双端 SHA-256 一致。逐项读数见 `docs/HOLDOUT_LEDGER.md` 观测 3。

### LOG-20260815-04：第四次观测——本项目的第一个 GO，来自部署形态而不是新模型

- 日期：2026-08-15
- 阶段/任务：R4.5，sealed 契约 v1.1 的第一次真实使用（代码冻结于 `06e4cc2`）
- 状态：解决（**产生了项目历史上第一个 GO，并把"发布判定该测哪种部署形态"固化下来**）
- 关联：LOG-20260815-03（第三次观测暴露的契约限制）、LOG-20260815-01（部署形态归因）

**判定：GO / deployment=candidate / form=merged，v1.0 与 v1.1 两套口径都是。**
...
**GO 归因于部署形态而不是 base 侧噪声**：把未合并候选（`candidate-003`）对同一份
`base-004` 重算，`p95_latency_ratio` 仍是 **1.9219 FAIL**。同一份权重、同一套行为
（两侧 `average_tool_calls` 都是 1.5000、任务指标都是 120/120），只差加载方式。

**这条改变了后续做法**：**发布判定必须测将要上线的那种部署形态。** 用未合并形态去判
一个打算合并后上线的候选，测的不是那个系统——三次 NO-GO 里至少有两次是这么产生的。

**这不是前三次被拒的那个候选**。它是同一份权重的另一种部署形态。三次 NO-GO 的结论
一个字不改，未合并形态在观测 3 与上面的复算里都仍然失败。

**SPEC §6 的第 6 条没有满足**：「最终候选独立重建后仍保持正向提升」需要一次独立重建
复验，**尚未进行**。自动门禁只实现前 5 条（v1.0）/ 8 条（v1.1）。因此这是
"**自动门禁 GO**"，不是"发布流程全部条件满足"。简历口径必须写成前者。

#### 必须披露的顺序问题

配对校验最初把 `schema_version` 当作配对字段，于是 v1.0 的 base 与 v1.1 的合并候选
无法配对。**我是在看到两侧的聚合指标之后才改的这条规则**——从那些指标可以算出
p95 比值 1.1265，也就是说我在改规则时已经能预见判定方向。这个顺序必须记下来，
由读者自行判断。

改动本身的理由与判定结果无关，且不改变任何一个门禁的算术：`schema_version` 描述的是
**报告格式**而不是实验条件。两次运行只要 `code_commit`、`uv_lock_sha256`、模型、
生成参数、数据集、receipt、prompt、工具 schema 全部相同，就是在同一条件下跑的。
把它当配对字段还会产生一个更糟的性质：**每次 schema 升级都要重跑 base 侧**，
为一次序列化变更烧掉一次封存 holdout 观测。

跨版本配对的健全性改由一条更强的约束保证：`SEALED_HASHED_FIELDS` 只能**追加**，
旧版本的字段集合必须是新版本的**真子集**——逐对校验 + 一条对整张表的结构性测试。
任何一版若改变（而非追加）字段含义，跨版本配对立刻被禁止。

另一条路径是给 base 侧也声明 v1.1 并重跑两侧（约 9 分钟 GPU、再消耗两次运行）。
我没有选它，因为那正是上面那个"更糟的性质"的实例。两条路径会给出同一批数字，
差别只在多烧两次观测。

#### 记账

第四次观测消耗 **2 次运行**（base-004、merged-candidate-004）。截至本条，封存
holdout 累计 **4 次观测 / 9 次运行**。`report_id`：base `85972fed…` /
candidate `a4ad8ee9…`，均在 `code_commit` `06e4cc2` 下产出。

#### 契约扩展本身的验证

合并候选的血统在真实产物上闭环：`merged_revision` 由「基座 revision + adapter 逐文件
哈希」复算得到 `00f51386…`，与合并脚本落盘的 provenance、与模型 pin 的 `revision`
三者一致。配对校验重算它并核对 `merged_from.base_repo/base_revision` 就是 base 侧
实际跑的模型——"这份权重确实由那对输入合成"因此是可验证的事实而不是一句声明。

### LOG-20260816-01：分布外评测证实 120/120 不是泛化——而且训练把表面鲁棒性从 30% 变成了 0%

- 日期：2026-08-16
- 阶段/任务：R4.5，评审 P0-1 的分布外任务集（`retail_ops_ood_v1_20260815`，60 条）
- 状态：解决（**改变候选选择与结果表述的做法；对刚拿到 GO 的候选是负面读数**）
- 关联：LOG-20260815-04（第四次观测的 GO）、LOG-20260815-01（部署形态归因）

**结果（gpu-5090，`code_commit` `d57f17f`）**：

| | base（零训练） | 合并版候选（拿到 GO 的那一份） |
|---|---|---|
| 封存 holdout（模板内） | 0.8583 | **1.0000** |
| **分布外（模板外）** | **0.2167** | **0.5833** |

**同一个候选从 100% 掉到 58.3%。** 逐类别看，训练不是均匀变好，而是**一半变好一半变差**：

| 类别 | base | 候选 |
|---|---|---|
| `scenario_ood`（工具面做不到 + 多实体） | 0.00 | **0.75** |
| `adversarial`（错订单号 / 脏字段 / 缺字段 / 工具诱导） | 0.35 | **1.00** |
| `expression_ood`（口语 / 寒暄 / 错别字 / 中英夹杂 / 极简） | **0.30** | **0.00** |

最尖锐的一条：**`code_switch`（中英夹杂）base 1.00 → 候选 0.00**。在 400 行中文模板上
做 SFT，把基座原有的多语言与口语鲁棒性挤掉了。候选在表达类的 20 条失败**全部**是
`premature_final_response`——正是 R3 那个老失败模式；换一种说法它就退回去了。

**核心结论（改变后续做法）**：

1. **训练教会的是"这种措辞出现时该退款"，不是"什么时候该退款"。** 冻结 holdout
   与训练集共用同一批 12 句模板，所以那 120 条只看得见被换来的那一半能力。
2. **但训练确实教会了真东西**：查证后拒绝做不到的请求（部分退款/换货/通融三类全对）、
   在脏数据与工具诱导下不乱来、政策违规 12→0。这些不是模板匹配能解释的。
3. **因此"SFT 有没有用"问错了**。这次 SFT 用**表面形式鲁棒性**换来了**任务结构与
   安全性**。今后报告 SFT 效果必须同时给出模板内与模板外两个数，只给前者是误导。
4. **候选选择与发布判定应把分布外读数列为必看项**，哪怕它暂时不是门禁。
   把它做成门禁需要先解决"OOD 集合会不会被过拟合"，那是独立决策。

**对第四次 GO 的影响**：**GO 不改**——它是对封存 holdout 的判定，门禁测的就是它设计
要测的东西。但这份读数必须与那个 GO 一起出现：`SPEC.md` §6 的五（八）项门禁里
**没有一项测表面形式鲁棒性**，而一个通过全部自动门禁的候选在模板外的表达变化上是
**0/20，且比不训练还差**。

**边界**：表达变体是**作者手写**的模板库（两台机器都没有 `TEACHER_LLM_*` 凭据），
度量的是"对我想到的那些变化的鲁棒性"，真实分布只会更宽；n=60，CI 约 ±12pp；
这个集合**不封存**，用它选候选就等于开始过拟合它——这一点与封存 holdout 的纪律不同。

`run_id`：base `8aada609…` / merged `023a03b9…`，manifest `afc41351…`，
60/60 可重放。产物 `reports/retail_ops/v1/ood/`。

### LOG-20260816-02：第四档 merged + vLLM——3.32× 是乘性的两段，一半根本不需要 vLLM

- 日期：2026-08-16
- 阶段/任务：R4.5，执行提示词 §7.2 的第四档（前三档已由正式评测产出）
- 状态：解决（**改变"延迟代价该归因给谁"的做法**；同时记入一条新的资源边界约束）
- 关联：LOG-20260815-01（部署形态归因：合并把单次调用耗时降 46%）、
  LOG-20260815-04（第四次观测的 GO）

**口径**：vLLM 0.27.1 装在**独立 venv**（Python 3.12），项目 `uv.lock` 一个字节未动
——`uv_lock_sha256` 在 `SEALED_PAIRING_FIELDS` 内。这一档是**旁证**：
不产出 run evidence，不进任何发布判定。输入是 12 条**公开** R1 qualification fixture
（不碰 holdout/dev 请求），同一份合并权重，同一套贪心契约，gpu-5090 物理 GPU 0。

**先证明三侧在做同一件事**：解析后工具调用 **12/12 一致**，归一化特殊标记后文本
**12/12 逐字相同**，三侧输出 token 总数**都是 390**。否则吞吐数字衡量的是另一个模型。

| | HF + NF4（当前服务栈） | HF + bf16 | vLLM + bf16 |
|---|---|---|---|
| 单流 mean / p95 (ms) | 675.97 / 1078.70 | 411.66 / 817.23 | **203.51 / 220.39** |
| 输出吞吐 (tok/s) | 48.08 | 78.95 | **159.70** |
| p95 / p50 | 1.70 | 2.21 | **1.10** |

**这条改变后续做法**：3.32× 是**乘性的两段**——去掉 NF4 得 **1.64×**，再换 vLLM 得
**2.02×**。**前一段不需要新依赖、不需要新引擎，栈保持冻结**（5090 有 32 GB，
4B 模型 bf16 才 8 GB，NF4 从来不是容量需要）。此前项目把延迟代价先归给"模型太大"、
再归给"部署形态"（LOG-20260815-01）；现在"部署形态"这一项还要继续拆，
**量化与引擎是两个独立旋钮，不得把整个差距记在任一个头上**。

**批量吞吐 1375.38 tok/s（12 并发冷启）是单流 HF+NF4 的 28.6×，但当前服务契约
拿不到它**——服务刻意串行、并发上限 1。这一档衡量的是**放弃了什么**。
prefix caching 值 **+57.5%**（对照组：关闭后第二遍与第一遍持平）。

**明确不声称**：(a) vLLM 未跑过任何评测任务，12 条上的一致 ≠ 60/120 条上行为一致，
要那个结论必须把 vLLM 引入项目依赖即让全部 sealed 证据失配；(b)「去掉 NF4 就能过
延迟门禁」是**假设不是结论**——门禁是比值、base 侧也是 NF4，验证它等于封存 holdout
的**第五次**观测，需用户单独决策；(c) `torch.compile` 仍未做；(d) n = 12。

**新的资源边界约束（gpu-5090）**：这台机器**没有 C 编译器**（只有 `cpp` 预处理器）。
而 torch 2.13 的前向必然初始化 triton，triton 的 `cuda_utils` 缓存键**只含源码与
triton 版本、不含 Python ABI**。两个 venv 的 triton 同为 3.7.1，因此 3.12 的 vLLM
运行**覆盖**了项目 3.11 的缓存条目，此后项目自己的 HF 评测路径全线崩溃且
错误信息（"找不到 C 编译器"）完全不指向真正的原因。已修复（zig cc 重建 + 给 vLLM
侧单独的 `TRITON_CACHE_DIR`）并复验。**今后在此机引入任何第二个 Python 版本的
深度学习环境，必须先隔离其 `TRITON_CACHE_DIR`。** 机制与证据见 `findings.md`。

### LOG-20260816-03：引擎替换不保行为——训练过的候选一致，零训练基座不一致

- 日期：2026-08-16
- 阶段/任务：R4.5，用户裁定"vLLM 走完整 evaluate 路径，只跑 dev + OOD"
- 状态：解决（**改变"换引擎"这件事的默认假设**；并纠正一条我此前写错的判断）
- 关联：LOG-20260816-02（第四档吞吐）、LOG-20260816-01（OOD 集合）

**先纠正**：此前记为"vLLM 走完整 evaluate 必须引入项目依赖、会让全部 sealed 证据失配"
——**字段找错了**。`uv_lock_sha256` 哈希的是仓库里的 `uv.lock` 文件
（`product_cli.py:1500`），不是实际装了什么包；换一个 venv 跑它**发现不了**。
这本身是个缺口：该字段声称锁定运行环境，实际只锁定了一个文件。真正的拦截是
`GenerationSettings.quantization: Literal["nf4"]` 加 `_require_backend_matches_pin`
的逐字段比较，而那是**对的**那种拦截。于是让 vLLM 也跑 bitsandbytes NF4
（两侧 bnb 均 0.49.2），**契约一个字未改，且引擎成为唯一变量**。

**结果（gpu-5090 物理 GPU 0，三组 × 两引擎，`replayable` 均 60/60）**：

| | HF | vLLM | |
|---|---|---|---|
| dev 60 / 合并候选 | 1.0000 | **1.0000** | 六项指标逐字段相同 |
| OOD 60 / 合并候选 | 0.5833 | **0.5833** | 逐类别、逐 kind、失败分布全同 |
| OOD 60 / 零训练基座 | 0.2167 | **0.2333** | **不一致**，差异全在 `colloquial`（失败 2→1） |

**这条改变后续做法**：**引擎替换必须逐模型验证，不能假定保行为。**
"换了个更快的引擎，模型还是那个模型"在本次实验里一半成立一半不成立。机制已定位：
基座在 OOD 上约 47 次调用里 44 次非法，输出本就在合法/非法边界上晃，两个引擎的 NF4
算子实现差异足以把边界样本推到另一侧；训练过的模型输出决断，推不动。
**连带后果**：发布门禁是比值、base 与 candidate 两侧都要测，因此若要在 vLLM 上做发布
判定，**base 侧读数会变**，不能沿用 HF 的 base 证据。

**吞吐（同为 NF4，纯引擎效应）**：三组 4.86× / 4.84× / 4.89×，稳定 ~4.85×。
**与 LOG-20260816-02 的 2.02× 不一致，两者都成立且必须一起引用**——那里是 bf16 对
bf16 的 12 条单轮 fixture，这里是 NF4 对 NF4 的真实多轮任务；HF 的 bnb NF4 前向要逐次
反量化，相对代价远大于 bf16。**"引擎快多少"不是一个数**，取决于量化与工作负载。

**明确不声称**：(a) 不是发布判定，dev 与 OOD 都不封存；vLLM 上的封存判定会是**第五次**
观测，需用户单独决策。(b) 两侧不同 commit（HF `d57f17f`/`007e506`，vLLM
`b573f23`/`d1f37bc`），期间只新增了 vLLM 后端与硬件 provider、未触碰 transformers 路径，
但严格说属跨形态对照而非配对判定。(c) `peak_memory_bytes` 跨引擎不可比。
(d) 未合并 adapter 未测（`VllmBackend` 对 adapter 硬失败）。(e) `colloquial` 的差异
只有 1 条任务，足以证伪"必然保行为"，不足以量化"会翻多少条"。

逐项读数见 `docs/ENGINE_SUBSTITUTION.md`。

### LOG-20260816-04：证据补记真实运行环境——`uv_lock_sha256` 从来不是环境锁

- 日期：2026-08-16
- 阶段/任务：R4.5，LOG-20260816-03 暴露的缺口的收口
- 状态：解决（**改变证据契约**：dev / OOD 运行证据新增两个字段）
- 关联：LOG-20260816-03（引擎替换）、LOG-20260810-02（sealed 字段集合冻结）

**缺口**：`uv_lock_sha256` 哈希的是仓库里的 `uv.lock` **文件**，不是实际装了什么包。
本轮三次 vLLM 评测跑在一个完全不同的 venv（Python 3.12 + vLLM）里，产出的证据却
逐字段声称用的是冻结依赖，**没有任何机制发现得了**。

**做法**：`BaseRunEvidence` / `OodRunEvidence` 新增 `inference_engine` 与
`runtime_env_sha256`，二者必须同时记录或同时缺失。**缺失读作"未记录"，
不是 "transformers"**——2026-08-16 之前的证据都没有这个字段，把缺失当成某个具体值
就是编造。

**关键实现选择**：**不**用 `schema_version` 承载这次演进。`CandidateRunEvidence`
早已把它当作**类型判别符**（固定 `"1.1"`），再叠演进语义会让同一个 `"1.1"` 表示两件事，
且会改变磁盘上已有候选证据的哈希投影。改成"取值为 None 即不参与内容哈希"：
旧证据加载后两字段为 None，复算逐位不变。**gpu-5090 上对 48 份已有证据全量复算，
0 不匹配。**

**这条改变后续做法**：往运行证据加字段的标准手法从"版本化投影"改为
"新字段取值为 None 时不入哈希"，因为前者会与已被占用的 `schema_version` 语义打架。
新字段一律登记进 `RUNTIME_PROVENANCE_FIELDS`，漏登记会让旧证据复算不出原值。

**过程中踩到并修掉的一个真缺陷**：第一版用 `importlib.metadata.distributions()`，
它沿 `sys.path` 搜索，量的是"当前能 import 到什么"而不是"这个解释器装了什么"。
同一个 vllm venv 里 OOD 记下 `60dd5028…`、dev 记下 `3a5c65b5…`——**这个字段成了噪声，
比没有更糟**，因为它看起来像个可比的标识。改按 `sysconfig` 的 purelib/platlib 扫描后
三次评测记下同一个摘要，且读数逐字复现（OOD 候选 0.5833 / OOD 基座 0.2333 /
dev 候选 1.0000）。

**未做**：`SealedEvaluationReport` 未加这两个字段——封存轨道至今只在 transformers
上跑过，改它需要 v1.2 版本化，属独立决策。

### LOG-20260816-05：独立重建复验通过，但"60/60"不是一个常数

- 日期：2026-08-16
- 阶段/任务：R5，`SPEC.md` §6 第 6 条——全项目最后一条未做的发布门禁
- 状态：解决（**改变 dev 读数的表述口径**，并新增一条关于可复现性边界的约束）
- 关联：LOG-20260815-04（第四次观测的 GO）、LOG-20260814-05（跨规模验证）

**做法**：同一份 SFT 配置 `retail_ops_v1_r4_round3_capacity_prompt_sft.yaml`、
同一份训练数据、同一个基座、同一组超参，只换 `--seed` 重训两次（seed 0 与 seed 1），
再对同一份 base 证据 `qwen3-4b-dev-base-002`（54/60）做配对评测。
**全部在 dev 上，未消耗第五次封存 holdout 观测。**

先排除混淆：`src/veritool_rl/training/` 最后一次改动是 2026-08-09（`c466b64`），
**早于**原 sft-006 的 8-14 训练；`train-export-004/sft.jsonl` SHA-256 一致；
解析后的 `config.yaml` 与原次逐行 diff **只差 `adapter_dir` 与 `output_dir` 两行**。

**结果**：

| 运行 | seed | adapter SHA-256 | dev task_success | 政策违规 |
|---|---|---|---|---|
| 原 `sft-006` | 0 | `8a49251f…` | 1.0000（60/60） | 0 |
| 重建 A | **0** | `c93c6698…` | 0.9667（**58/60**） | 0 |
| 重建 B | 1 | `069cdf1c…` | 1.0000（60/60） | 0 |

**第 6 条满足**：两次重建都显著高于零训练基座的 54/60，政策违规三次都是 0。
正向提升不是 seed 彩票。第四次观测因此可表述为「SPEC §6 六条全部满足」。

**这条改变后续做法（两点）**：

1. **训练是本项目唯一不能逐位复现的环节。** 代码、数据、配置、seed 全同，
   产出的权重逐位不同（GPU 非确定性归约）。此前项目在 `bundle_sha256`、`report_id`、
   qualification 链路上都做到了逐字节复现，并把"可复现"当成全局属性——**它不是**。
   今后凡是引用训练产出的读数，必须按"这是一次抽样"处理，不得当成常数。
2. **dev 读数的表述口径改为区间**：「**58–60/60，三次同配置运行**」。
   写单点 60/60 而不写区间，等于把一次运气好的运行当成常数。
   波动**全部**落在 `refund_recovery`（10/10 → 8/10 → 10/10），正是"后训练独占贡献"
   的那一类；失败类型也从基座的 `policy_violation` 变成 `recovery_failure`——
   **训练稳定消除了违规，但"失败后重试"在满分附近仍是边际能力。**

**明确不声称**：(a) 没有在 holdout 上重建，拿重建权重跑封存集会是第五次观测；
(b) n = 3 次运行，足以证伪"逐位可复现"、足以支持"不是 seed 彩票"，**不足以给方差的
置信区间**；(c) 复验证明的是流程能重现结果，**不证明结果能泛化**——同一候选在分布外
集合上仍是 0.5833。

逐项读数见 `docs/REBUILD_VERIFICATION.md`。

### LOG-20260816-06：R5 收口——审计与故障矩阵各暴露一个真缺陷

- 日期：2026-08-16
- 阶段/任务：R5 公开交付与求职收口
- 状态：解决（**新增两道常设门禁**）
- 关联：LOG-20260816-05（重建复验）

只记两件改变做法的事，其余（文档重写、格式统一、lint 扩展、双语版本）属常规执行，
由 git history 与 `progress.md` 承载。

**一、把"公开边界"从文档承诺变成常设门禁。** 此前 `.gitignore` 与逐个配置的治理测试
覆盖的是**已知**文件；新增 `scripts/ci/audit_public_release.py` 扫 `git ls-files`
**全集**，因此新增一个从未被任何测试覆盖的文件时仍然拦得住。已进 CI。

**关键设计（第一版是错的）**：绝对路径与 holdout 真值两项，第一版按**文件文本**扫描，
产生 14 条误报——配置注释里的溯源、负测试里的坏输入、源码里的字段名全被当成缺陷。
改为扫描 YAML/JSON **解析后的取值**后归零。**"提到"与"取值"是两件事**，
今后写这类静态边界检查一律按取值扫描。豁免清单被 `PATTERN_FIXTURE_ALLOWLIST`
钉死为恰好两个文件（定义模式的脚本与验证它的测试），并有测试断言它不能变长——
豁免是这类审计最容易被悄悄放大的地方。

**二、故障矩阵暴露了 SPEC §9 只做到一半。** `teacher_client._classify_retryable`
一直把超时归类为可重试，但 `from_route` 构造 SDK client 时**从未设置超时**，
落到 openai SDK 默认的 600 秒。240 条轨迹是 519 次请求，一次挂起就是 10 分钟静默停摆，
而采集脚本看起来像在正常工作。已改为显式 60 s 超时 + `TEACHER_MAX_RETRIES = 2`。
**分层重试必须让乘积可预测**：SDK 层上限与采集层上限是两个分开的旋钮，不得只设一个。

**做成矩阵而不是清单的理由**：`docs/FAULT_MATRIX.md` 的每一条都写明
`文件::测试名`，`tests/test_fault_matrix.py` 解析文档逐个断言其真实存在。
故障矩阵的失效方式不是"写错了"，而是"越写越好看、实际覆盖不动"。

**未做**：跨模型/商业 API 参考上限（无 `TEACHER_LLM_*` 凭据、未授权新模型下载）、
演示视频、推送公开仓库（仓库仍无 remote，属用户动作）、第五次封存 holdout 观测。

### LOG-20260816-07：外部审阅两轮——缺陷全部落在本项目自己的卖点上

- 日期：2026-08-16
- 阶段/任务：R5 收口，用户要求「由一个独立面试官角色审核通过才算完成」
- 状态：解决（**改变两条工程实践**）
- 关联：LOG-20260816-05（重建复验）、LOG-20260816-06（审计与故障矩阵）

**做法**：派一个**不带本会话任何上下文**的 reviewer，只给仓库路径与一句岗位设定
（「资深技术面试官，评估这个个人项目是否值得进下一轮」），要求它自己跑门禁、
自己抽查数字、给 PASS/FAIL 与逐条阻塞项。两轮。

**第一轮：PASS 8/10，4 条阻塞项。四条全部属实，且全部落在「数字纪律」上**——
而数字纪律正是这个项目对外声称的核心价值。这一点比任何单条缺陷都重要：

1. **R5 的评测证据只在 GPU 机器上**。项目最新、最关键的一条主张（SPEC §6 第 6 条满足）
   恰恰是唯一无法核对的。其余每个阶段的证据都同步回了本地，唯独最新这次没有。
2. **「唯一事实源」自己是漂移最久的文件**。`HOLDOUT_LEDGER.md` 正文仍写着
   「任何新的发布判定都需要第三次观测」——因为它**不在**漂移扫描的 `checked` 列表里，
   只有表头被断言。**把一份文件指定为唯一事实源，却不扫描它，等于没有事实源。**
3. **teacher 成本把两批付费采集焊在一起**：批次 2 的通过率 + 批次 1 的请求数与成本，
   而且没有任何文档给出两批总计（实际 1045 次 / 约 $0.111，不是 519 次 / $0.055）。
4. **训练数字混了三次运行**，README 甚至写出了模型卡明确拒绝写的那个数值。

**第二轮：PASS，4/4 解除，8 → 8.5**。reviewer 用**审阅之前**的 commit 里记录的
`run_id` 去比对同步回来的文件，并做了篡改测试——它没有采信自述。

**这条改变后续做法（两点）**：

1. **凡是文档里的数字，必须绑到一个可执行的校验上，不能靠人工同步。**
   本轮修完第一遍之后，我自己又漏了 5 处残留（两条简历 bullet、系统卡的成本行、
   执行计划的未做原因、模型卡残留的「显著高于」）。同一组数字散在 6 个文件里，
   手工同步必然漏。据此新增的绑定：测试数绑到 `pytest --collect-only`、
   teacher 两批绑到「出现批次 1 成本就必须出现批次 2 或总计」、
   容器体积与断网验证绑到实测值、故障矩阵绑到 AST 解析。
2. **凡是对外宣传的机制，必须至少被完整行使过一次，否则不得写进机制表。**
   项目一直宣传「运行报告全字段自哈希 + 逐产物 SHA-256 绑定」两个机制。
   前者一直在用；**后者从未对任何一次真实 GPU 运行行使过**——它需要私有产物在场，
   而私有产物只留在 GPU 机器上。已把 R5 私有产物同步到本地完整跑通
   （含改 `trajectories.jsonl` 一个字节被拒），并在机制表里写明
   「这半个机制读者无法自行复核」。同理，`Dockerfile` 注释里「几十 MB」是个
   从未构建过的估计，实测 **1.05 GB**，差一个数量级；镜像已首次实际构建，
   并在 `--network none` 下跑通全链路——**这比那份从未运行过的 CI workflow 更强**。

**未被审阅动摇的三条**：reviewer 明确指出它找不到任何 overclaim（无 remote 的事实、
禁止「生产可用」的清单、演示视频未录、注入数字来自脚本探针而非真实模型，
全部在文档里主动声明）；测试不是摆设（747 个测试函数里只有 9 个无 assert，
且都是合法的 must-not-raise）；以及 2026-08-15 那次「看到指标之后才改配对规则」
的自我披露，被评价为优于多数工业团队的实践。

**分数的天花板不在工程**：reviewer 的锚点是「10 分 = 这套严谨性 **加上** 一个能扛住
分布漂移或真实流量的结果」。本项目的结果本质上是负的（SFT 用表面鲁棒性换了任务结构），
领域是 2 工具 / 6 类 / 12 句模板——**这才是 8.5 与 10 之间的距离，不是再补几个测试**。

### LOG-20260817-01：泛化修复——把「表面形式 → 动作」的捷径打断，代价是模型更爱动手

- 日期：2026-08-17
- 阶段/任务：R6，回应外部审阅指出的天花板（「差距不在工程，在有一个扛得住分布漂移的结果」）
- 状态：解决（**改变训练数据的构造方式与评测集的治理方式**）
- 关联：LOG-20260816-01（v1 打掉了 120/120）、ADR 0005、`docs/GENERALIZATION_FIX.md`

**先诊断**：把 v1 的 `expression_ood` 按子类拆开发现，训练把零训练基座**本来会做**的两类
打没了（`code_switch` 1.0→0.0、`colloquial` 0.5→0.0），20 条失败全部是
`premature_final_response`。机制是 240 条 train 全部来自 12 句「请核实…」的书面祈使句模板，
模型学到的是**表面形式 → 动作**的触发器。

**干预**：LLM 措辞池（267 条，40 次请求 $0.0061）做训练增强，**只改 user 的第一句话**。

**结果（封存分片，只观测一次，代码冻结 `786fd8c` 后跑，运行内容观测前固定）**：
...
**独立迁移检查**（作者手写的 OOD v1，生成过程完全不同，本轮从未用于选择）：
`expression_ood` **0.00 → 1.00**（五个子类全满分），总分 0.5833 → **0.8667**。

**这条改变后续做法（三点）**：

1. **训练数据的表面形式多样性是一个必须显式管理的量。** 此前项目把 12 句模板当作
   「数据集的一个属性」，实际上它是**一个未被控制的变量**，而且是决定分布外表现的
   主要变量。今后新建任务集时，请求文本的多样性要与场景配比一样被写进契约。
2. **评测集的治理升级**（ADR 0005）：判断泛化的集合必须按哈希切分、训练与评测素材
   逐条互斥、留一个只观测一次的分片。外部审阅问过「什么阻止你对着它调」，
   此前只有一句承诺，现在有机制。
3. **收益与代价来自同一个改动，必须一起报。** 措辞增强让模型更倾向执行，
   于是「该动手时动手」（`eligible` 0.10→1.00）与「不该动手时也动手」
   （dev 新增 2 次 `refund_denied_window` 违规、OOD v1 的 `partial_refund` 1.00→0.00）
   同时发生。**两个只差 oversample 的候选出现完全相同的 dev 失败签名**，
   因此这个代价可以确定地归给措辞增强本身，不是 oversample。

**一个关于「规则」的教训**：候选选择规则在看到读数前写定并提交（`3416cee`），
但规则第 3 条里塞了一句对结果的预期（「选 sft-007，OOD 更好」），而实测反过来。
按字面执行会选中一个被严格支配的候选。已按实质择优选 `sft-008`，
并把偏离字面规则的事实与理由写进 `task_plan.md`。**别在规则里塞对结果的预期。**

**明确不声称**：(a) 不是「泛化了」，是「对同一个生成器产出的、训练从未见过的措辞更
鲁棒了」——对任意真实用户输入的鲁棒性没有被测量；(b) **本轮没有消耗第五次封存 holdout
观测**，四次观测的结论一条未改；(c) 自变量不纯，sft 行数 400→1600 使梯度步数
75→~300，「说法更多样」与「训练更久」在这个设计里分不开；(d) n=60，逐场景 n=10。

### LOG-20260817-02：演示视频与「读数指南」——两件让项目可被外人检验的事

- 日期：2026-08-17
- 阶段/任务：R5 补完 + 用户追加要求
- 状态：解决（**改变对外交付的最低要求**）

只记这一条改变做法的：**凡是对外的演示，生成过程必须与内容生成分离。**

`scripts/ops/capture_demo_transcript.py` 执行真实命令并存 transcript，
`scripts/ops/render_demo_video.py` **只读 transcript、不执行任何命令**
（有测试断言它只调用一次 ffmpeg，且无 `os.system`/`eval`/`exec`）。
两步分开之后，「视频里的输出是真跑出来的」不再是一句承诺，而是一个结构性事实。

顺带修掉一个自己的未验证数字：`Dockerfile` 注释写镜像「几十 MB」，从未构建过；
实测 **1.05 GB**，差一个数量级。镜像已首次实际构建，并在 `--network none`
（完全断网）下跑通全链路——**这比那份从未运行过的 CI workflow 更有说服力**。

另新增 `docs/READING_THE_NUMBERS.md`：逐个解释项目里高得可疑的数字
（120/120、三项同时为 0、99.2%、可重放 100%、封存分片 60/60），
每一个都配「为什么可能」「旁边那个不好看的数」「不能支持什么」，
并对称地列出「哪些数字低得可疑」。**对策不是把数字改小——那是另一种造假。**

### LOG-20260817-03：v1.1 门禁缺配对证据时**看起来像模型失败**——分层修法

- 日期：2026-08-17
- 阶段/任务：R6，第五次封存 holdout 观测的过程中踩到
- 状态：解决（**改变一条 API 设计原则**）

**踩坑经过**：跑第五次观测的 `release` 时漏了 `--baseline_trajectories` /
`--candidate_trajectories`，产出的报告是 **`NO-GO`**、失败门禁 `success_delta_ci_lower`、
观测值 `insufficient_paired_evidence`。那份报告**读起来像模型没通过统计检验**，
实际是命令少了两个参数。补上参数重跑得到 **GO**（CI 下界 +0.0583）。

**这条改变后续做法**：**「缺证据」与「没通过」必须在产出报告之前就分开。**
一份会被直接抄进交付材料的 `NO-GO`，必须来自模型而不是来自操作失误。

**修法是分层的，两层语义不同**：

- **库层** `_paired_ci_gate` 保持 **fail-closed**（缺证据判 FAIL，不抛错）。
  这是有意的，且早有测试锁定：它要支持「拿一份只有聚合量的公开报告复算门禁」，
  纯函数在缺一个可选输入时崩掉会让那件事做不成。**我第一版的修法直接推翻了它，
  打红 7 个既有测试——那些测试是对的，我错了。**
- **CLI 层** `_paired_outcomes` **硬失败**：操作者明确配了 `gate_schema_version: 1.1`，
  缺证据就是配置错误。此前它对「只给一侧」已有保护，漏掉了「两侧都不给」。

**一般化的原则**：同一个「缺输入」在库层和命令行层的正确语义可以相反。
库层要能在退化输入下给出保守答案；命令行层知道操作者的意图，该拒绝就拒绝。

### LOG-20260817-04：第五次封存 holdout 观测——最好的候选终于经过了门禁

- 日期：2026-08-17
- 阶段/任务：R6 收口
- 状态：解决（**GO，两套口径都是**）
- 关联：LOG-20260817-01（泛化修复）、`HOLDOUT_LEDGER.md` 观测 5

**为什么消耗这次观测**：项目的整个主张是 `build → evaluate → release → serve` 这条链路，
而 R6 的最终候选 `sft-008` **从未经过发布门禁**——「你修好了泛化，那它过不过你自己的
门禁？」这个问题的答案是「我没测」。那是当时最大的结构性短板。运行内容与三种判读
在跑之前写进 `task_plan.md` 并提交（`705a066`）。

**结果**：117/120（base 103/120）、政策违规 11→2、非法调用 5→0、
`p95_latency_ratio` **1.0203**、`success_delta_ci_lower` **+0.0583**。
**GO / candidate，v1.0 与 v1.1 都是。**

**必须一起说的两条**：(a) 候选**不是满分**——117/120 且有 2 次政策违规，
而 `sft-006` 那次是 120/120、0 违规；**这是 R6 措辞增强的代价在模板内的体现**，
与 dev 上观察到的完全一致。(b) base 侧 p95 这次比第四次慢 6%，
而门禁是比值——**更慢的 base 等于给候选放宽了门禁**。这条波动此前一直被记为风险，
这次是往有利方向偏，必须照说。

**封存 holdout 到此不再有「未观测」状态**：五次观测全部消耗，此后任何判定
都必须基于一个新的封存集合。

### LOG-20260817-05：外部审阅发现我的评测集**比训练集更容易**——而四个文件都写着相反的话

- 日期：2026-08-17
- 阶段/任务：R6，第三轮独立审阅
- 状态：解决（**改变评测集构造的验收方式**）
- 关联：ADR 0005、LOG-20260817-01

**发现**：OOD v2 的状态空间在三个维度上都比冻结集合窄——每条状态只有 **1 个订单**
（冻结集合 1–5）、期限余量只有 **±6 一种**（冻结 7 种）、订单状态只有 **3 种**（冻结 7 种）。
也就是说它**更容易**，而 `READING_THE_NUMBERS.md`、`GENERALIZATION_FIX.md`、
`ood_v2_tasks.py`、两份 build 配置**都写着「唯一自变量是顾客怎么说」**。

**更要命的一条**：没有干扰订单，「退错订单」这一整类失败在结构上**根本测不出来**，
于是「六个场景能暴露见谁都退款的捷径」这个论证比声称的弱。

**为什么我没发现**：我写了一条叫
`test_the_business_contract_matches_the_frozen_one` 的测试——名字承诺比对冻结契约，
实现却只是**抽一条样本看五个符号**。它的 `order()` 辅助函数是
`next(iter(orders.values()))`，**恰恰因为没有干扰订单才能工作**。
一个名字令人安心的测试，站在了本该做的那个检查的位置上。

**这条改变后续做法（两点）**：

1. **评测集的「与训练同分布」不能靠人肉核对，必须由测试直接构造两边来比对。**
   已换成 `test_the_state_space_matches_the_frozen_contract`：它构造冻结任务集，
   逐维度断言 OOD 的覆盖不窄于它（已做突变验证——把状态空间改回旧版立刻变红）。
2. **需要第二次测量时，生成新素材，不得复读已封存的分片。**
   修正状态空间后**换了一份全新措辞池**（`bank-002`，与 `bank-001` 训练措辞及
   真实训练文件均零重叠、已实测），旧分片退役并保留其唯一一次读数。
   OOD 封存分片也从此有了自己的台账（`OOD_SEALED_LEDGER.md`），
   与 `HOLDOUT_LEDGER.md` 同规格——此前它的「只观测一次」只是一句 Markdown 里的 grep。

**修正后的读数（新分片，只观测一次）**：零训练基座 0.7667、旧候选 `sft-006` **0.7167**
（**低于基座**）、新候选 `sft-008` **1.0000**。结论不仅站住，而且更硬——
旧候选在一个与训练同难度、措辞全新的集合上比不训练还差。

**另外三条同一批发现的缺陷**：`dataset_version` 被写死成 v1 的字面量（v2 的报告
声称自己属于 v1，两个数据集的读数在同一张表里不可区分）；
`test_the_generalisation_fix_is_never_quoted_without_its_cost` **极性反了**
（触发条件要求代价已经在场，于是只写收益的文档被直接跳过）；
定稿简历 bullet 里的测试数停在旧值（计数守卫的正则只匹配英文形式，中文「N 项测试」逃逸）。
三条都已修并补测试。

### LOG-20260817-06：**断言「某句话必须在场」的测试，会在那句话过期时把它焊死**

- 日期：2026-08-17
- 阶段/任务：R6 收口 / R7 独立重建复验的准备
- 状态：解决（**改变治理测试的写法**）
- 关联：LOG-20260817-05、ADR 0005

**发现**：第四轮外部审阅抓到 `test_r6_never_claims_a_release_decision` 在强制一段
**已经变成假话**的文档措辞。审阅的原话是「比名不副实更糟：它在强制一个谎言」。
本轮准备第六次观测时，同一个失败模式**又出现了一次**：
`test_the_fifth_observation_was_declared_before_it_ran` 钉的是第五次观测预注册的原话
（"运行内容（三个，固定，不得增减）"等等）。第六次观测把 `task_plan.md` 的那一节换掉后，
它拦的就不再是"没有预注册"，而是"预注册的措辞和上一次不一样"。

**这不是"文档写错了"那一级的问题。** 测试本该是防线，此时却成了帮凶——
它把一段过期表述**焊在**仓库里，谁想把它改成真话谁的 CI 就红。
而且这个失败模式**有繁殖性**：每一轮的预注册都会诱使人再写一条"这一轮的原话必须在场"。

**一般化的教训（这条改变后续做法）**：

> **边界类断言应该断言当前边界的语义，而不是某一段特定措辞。**

具体到这次的两条改法：

1. 「预注册必须写明三种以上结果」——断言**结构**：`判读规则` 一节里加粗的结果分支 ≥ 3。
   结果叫什么名字（GO / NO-GO / 复现 / 不复现）由当轮决定，"必须预先写明三种以上"
   这条纪律不由当轮决定。
2. 「先写定再跑」——断言**文件系统上的投影**：
   本轮已落盘的每一个运行目录，都必须出现在 `task_plan.md` 里
   （`test_every_declared_run_directory_is_actually_declared`）。
   目录还没生成时它自动为真，生成之后才开始有约束力。这比任何措辞检查都更接近
   要防的那件事——**跑了但没声明过的运行 = 事后挑选的机会**。

**同一天发现的第三种形状**：`README.en.md` 一直在
`test_no_active_doc_restates_a_stale_observation_count` 的 `checked` 列表里，
但整张 `stale` 词表**全是中文**，于是英文侧的 "Of four observations"、
"observed only four times" 一路没被拦住。**扫描覆盖了文件，却没覆盖它的语言。**
已补英文词条。

**代价与边界**：结构性断言比字符串断言弱——它拦不住"写了三个分支但内容是废话"。
这一条只能靠人读。**该测试保证的是纪律的形状在场，不是纪律的内容正确。**

### LOG-20260817-07：最终候选的独立重建——复现了，但它同时把 R6 的账单改大了

- 日期：2026-08-17
- 阶段/任务：R6 收口 / `sft-008` 的独立重建复验
- 状态：解决（**复现；同时推翻一条归因，并把两个头条数字改成区间**）
- 关联：LOG-20260816-05（第一轮复验）、LOG-20260817-01（泛化修复）、
  `HOLDOUT_LEDGER.md` 观测 6、`OOD_SEALED_LEDGER.md` 观测 2

**为什么做**：`SPEC.md` §6 第 6 条的独立重建复验当初只在 `sft-006` 上做、且只在 dev 上做。
**最终候选 `sft-008` 从未被独立重建过。** 同时 R6 的头条读数（封存分片 1.0000）
只来自**一份**措辞池的一个分片——单一 provider、单一 prompt、单一批次。

**做法**：同一份 SFT 配置（一个字未改，有测试断言它只有一次提交）、只把 `--seed` 换成 1
重训；另生成一份全新措辞池 `phrasing-bank-003`（$0.0061，与旧池及**真实训练文件**
交集全为 0，实测）作为**从未观测过**的分布外判据。八个运行与三条判据（A/B/C）
在跑之前写进 `task_plan.md` 并提交（`2c2c73b`）。

**判据结果**：A ✅（dev 60/60、0 违规）；B ✅（+0.2500 与 0.0000）；
C = **GO，两套口径**（113/120、7 次政策违规、`success_delta_ci_lower` **+0.0083**）。
**按事先写定的规则 → 复现。**

**但这一轮真正的产出是三条负面/修正性发现，它们改变后续做法**：

1. **「那 2 次政策违规是措辞增强的确定代价」这条归因不成立。**
   原论据是 `sft-007` 与 `sft-008` 失败签名完全相同——**但两者共用一个训练 seed**。
   换 seed 后 dev 上违规为 0。**把 seed 当成已控变量，是那次归因里没被发现的漏洞。**
   今后凡是「两个只差 X 的候选表现相同 → 归因于非 X」这类论证，**必须先确认 seed 也变过**。
2. **代价没有消失，而且在 120 条上比 dev 上大得多**：同配置两次运行分别 2 次与 **7 次**
   违规，全部是同一签名 `refund_denied_window`（那一类 20 条错 7 条，其余五类 20/20）。
   **dev 的那一类只有 10 条，结构上看不见这个量级。** 此前文档写的「2 次」
   是两次运行里较好的那一次。
3. **dev 与封存集把这两次运行排成了相反的顺序**（dev 上重建更好，封存集上原候选更好）。
   这是「dev 不能替代封存集」的**事后证据**，也是"不要用 dev 挑同配置的不同运行"的直接理由。

**据此改掉的表述（全部已同步到 README ×2、四份文档与两份台账）**：

| 此前 | 此后 |
|---|---|
| 分布外封存分片 **1.0000** | 「**1.0000 与 0.9833**，两份独立生成的措辞池各一次」 |
| 封存 120 条 **117/120、2 次违规** | 「**113–117/120**，同配置两次运行；违规 **2 与 7**」 |
| 「独立重建复验已做」 | 「做了两轮，**第二轮覆盖最终候选**，含 dev + 分布外 + 封存 120」 |

**一个必须写下来的门禁性质**：观测 6 的候选**违规比观测 5 多**，却照样 `GO`——
因为门禁比的是与 base 的差（11 → 7 即通过）。**这是门禁定义本身的性质，不是这次的意外。**
一个在封存 120 条上给 7 个过期订单退了款的候选，在真实退款场景里不可接受。
**"过了门禁" 与 "可以上线" 之间的距离，这一次有了具体的数字。**

**资源与边界约束变化（一并记）**：封存 holdout 的观测次数自 2026-08-17 起
**不再是硬约束**（用户明确）。但**结果永远不得反馈进开发、调参、候选选择或
prompt/parser 修改**——那条限制来自统计学，不来自资源稀缺。
OOD 封存分片的规矩不变：**一份素材只观测一次**，需要第二次测量就生成新素材。

### LOG-20260818-01：防漂移机制被连判两次「换了形状的黑名单」——正确的形状是**白名单 + 限定作用域 + 自己的回归语料**

- 日期：2026-08-18
- 阶段/任务：R6 收口 / 外部审阅第五、六轮
- 状态：解决（**改变治理测试的设计方法**）
- 关联：LOG-20260817-06（钉住会过期的话）、LOG-20260817-07

**发生了什么**：「活动文档不得复述封存 holdout 观测总数」这条规矩，我做了三版：

1. **v1 手工字符串黑名单**（"已消耗两次观测"、"已消耗三次观测"…）+ 手写扫描文件列表。
   审阅第五轮判：**它永远落后当前状态一代**，且新增文档默认不在保护内。
2. **v2 结构化枚举**：扫描范围改为 `git ls-files` 派生（这一半是对的），
   但规则仍是五条手写正则。审阅第六轮用 **14 句自然的同类表述探测，漏了 12 句**——
   动宾倒装（「观测了六次」）、`累计`/`总数为`、以及**全部英文**。
   原话：「本质仍是黑名单，只是带了通配符。」
3. **v3（现在）**：三个改动一起才成立——
   - **限定作用域**：只在句子确实在谈「封存 holdout」时才管，于是可以严格；
   - **白名单**：允许的是一小撮封闭的语法标记（前/头/第/再…、first/another/the），
     其余一律禁。**枚举合法形式比枚举违法形式收敛得多。**
   - **给检测器自己上测试**：把审阅探出的每一句漏检钉成回归语料
     （`_MUST_BE_CAUGHT`），另配一组必须放行的反例（`_MUST_BE_ALLOWED`）。

**一般化的教训（这条改变后续做法）**：

> **凡是"禁止某类表述"的治理测试，枚举违法形式必然漏；应当限定作用域后枚举合法形式，
> 并且给这个检测器本身配回归语料——否则"它到底拦得住什么"只是一句声称。**

推论三条，都已落地：
- 规则**必须覆盖仓库使用的每一种语言**。审阅在同一件事上抓到过两次
  「扫描列表覆盖了文件、词表却没覆盖它的语言」。
- **风险最高的文字必须有机器约束。** 定稿简历 bullet 是全仓最会被外人读到的一段，
  却一直只靠人记得去看同文件里的「不可写清单」——审阅能把该清单明令禁止的两句
  原样塞进去而全仓测试全绿。现在清单的第一列被**解析出来直接绑到** bullet，
  另有「读数配对」检查（写了 117/120 就必须同时有 113/120）。
- **守卫的作用域要与它要防的事对齐。** 文件级检查挡不住"同一文件别处还留着对照"，
  第六轮正是从这个洞把 bullet 改回择优挑数而无一测试变红。

**另一件同轮发现、值得单独记的事**：项目对外写「N tests passed 全绿」，
而**干净 clone 上是 6 failed**——原因是缺少不随仓库分发的私有产物时，
三条 `assert checked >= 1`（拒绝空过，本身是对的）与两条 BFCL 测试直接红，
外加一条硬编码 `.venv/bin` 到 PATH 的**真 bug**。已改为「产物根不存在则 skip 并说明、
根在却加载不出才红」，真 bug 已修；干净 clone 现在 **0 failed**。
**但两个环境跑出不同数字这件事本身要写进对外口径**，并由测试保证那句披露不被删掉。

### LOG-20260818-02：连续三轮之后承认——**这类守卫做不到完备，正确的做法是换判别式并写清边界**

- 日期：2026-08-18
- 阶段/任务：R6 收口 / 外部审阅第七轮
- 状态：解决（**改变治理测试的判别式，并改变对它的表述**）
- 关联：LOG-20260818-01、LOG-20260817-06

**发生了什么**：同一条规矩（活动文档不得复述封存 holdout 观测总数）做到第三版，
连续两位审阅者用**新造的句子**把它击穿——第六轮 14 句漏 12，第七轮 29 句漏 26。
第七轮的判词是：这呈现出**对审阅者探针过拟合**的模式，
「每轮把上一位用过的那几句钉成回归语料，下一位换一批新句子照样漏」。

**这条判词是对的，而且它指出的问题比漏检本身更严重**：v1/v2 的 docstring 写着
「其余一律禁」，而实现是六条手写正则的 alternation——**声称的机制与实现不符**，
这正是本仓库反复在别处抓的那类缺陷，只是这次犯在治理代码自己身上。

**改法（三件事，缺一不可）**：

1. **换判别式**：从「枚举总数的说法」改为「**那个不该被复制的值**」——
   值从台账现算（`_ledger_observation_count`）。要拦的是「台账拥有的那个数被在别处
   又写了一遍」，无论量词、语序、中英文。**这条不由"上一位审阅者说了什么"定义。**
   实测：两轮审阅的 20 句探针全中，11 句合法相对指代全放行，真实文档零误报。
2. **白名单只放语法，不放 claim 形状**：相对/序数标记、指向台账某一条的序数指针
   （`观测 2`）、以及那个数字根本不在数观测（日期、小数、比分、哈希、标识符）。
   白名单不全只造成**误报**——逼人换个写法，方向是安全的。
3. **把 docstring 改成实话**：逐条写明挡不住什么（把总数写成算式、
   或完全改写成不含数字的错误陈述）。**那一层由「台账是唯一事实源」加人工审阅负责，
   没有机器保证。**

**一般化的教训**：

> **自然语言里的"不得复述某个事实"做不到完备的机器保证。**
> 能做到的是：①把判别式绑到**事实本身**（值、产物、另一份清单），
> 而不是绑到"人可能怎么写"；②在**有界的高风险面**上改用穷举白名单；
> ③把边界写进 docstring，不要用"其余一律禁"这种话掩盖枚举的本质。

**同轮落地的两个"有界高风险面"穷举白名单**，它们才是真正关得死的：

- **定稿简历 bullet 里的每一个数字，必须在同一份文件别处有出处。**
  这把 `RESUME_EVIDENCE.md` 自己那句散文（「两版都只用 §1 的数字」）变成可执行约束。
  第七轮用「任务成功率 **97.5%**」（= 117/120，即两次运行里较好的那次）绕过了字面量
  配对表且全仓全绿；溯源规则当场把它抓住，**因为它不认形状，只认出处**。
- **「还没做」的清单与阶段状态源互为校验。** §5 曾落后两条（LLM 改写、演示视频），
  而 §5 是全仓唯一约束「什么还不许写进简历」的清单——它落后就等于这个机制失效。

**另外两条同轮更正**：对外写的干净 clone 基线（1047 passed / 44 skipped）在算术上
**不可能成立**（1047+44 ≠ 收集总数），而当时的守卫只查字符串在场、不校验任何数字。
现在断言 `passed + skipped == 收集总数`。`SERVING_FORM_COMPARISON.md` 里
「合并形态根本无法获得判定」是写于契约 v1.1 之前的现在时陈述，早已变假，
已加时态补记——它承载的恰好是「GO 归因于部署形态」这条头条论证。

### LOG-20260818-03：**「真正关得死」是我上一条自己刚写下的新过度承诺**——外部审阅当场破掉

- 日期：2026-08-18
- 阶段/任务：R6 收口 / 外部审阅第八轮
- 状态：解决（**改判别式 + 全面去绝对化**）
- 关联：LOG-20260818-02（本条更正的对象）、LOG-20260818-01

**发生了什么**：LOG-20260818-02 里我写「同轮落地的两个有界高风险面穷举白名单，
**它们才是真正关得死的**」。审阅第八轮在几分钟内破掉其中之一，两种写法各一次、
全仓 1095 条测试全绿：

```
封存 120 条上只错了 3 条、政策违规仅 2 次      → 全绿
封存 120 条上任务成功率约 98%、政策违规个位数  → 全绿
```

两句都把较差的那次运行（113/120、7 次违规）挖掉了，而「只错 3 条」「约 98%」
对两次运行都不成立。溯源规则放行的原因很朴素：`3` 在本文件别处出现过
（0–999 里有 198 个整数出现过），`98%` 来自一句完全无关的「他人占用 0%–98% 不等」。

审阅的判词是：**这是第三轮同一物种的缺陷——docstring 承诺 > 实现**，
而且这次犯在我自己刚宣布的成果上。

**改法**：把配对规则的键从**字面量**换成**主题**——
§3 里凡是提到「封存 120」的句子，必须同时给出两次运行的读数。
改写躲得开数字，躲不开主题词。两次逃逸现已复现为红。

**但这一条真正改变做法的不是那个规则，而是下面这句**：

> **不要在长期档案里写"关得死"「一律」「永远」「零误报」这类绝对化断言。**
> 它们的期望收益是负的：审阅者会把全部精力用来找反例，而且**总能找到**——
> 连续四轮，每一轮的头号发现都是我某句绝对化表述的反例。
> 正确写法是把**边界**写出来：挡得住什么、挡不住什么、代价是什么。

据此本轮做了三件事，**没有新增任何一条约束**：

1. 把这类断言逐条降级为有界陈述（测试 docstring、README ×2、指南、本档案）；
2. 把两条守卫的键从字面量换成主题/出处，**替换而不是叠加**；
3. 把「过期值也挡得住」这句假话改成实话——实测 8 句过期值全漏，
   试过的默认拒绝方案在真实文档上 14 处误报，那条路是治标，不走。

**同轮修掉的一处实质错误（不是措辞问题）**：README 中英文都把分布外封存分片的
最小样本格写成 **n=3**，而台账（唯一事实源）写着按措辞去重后是 **n=1**
（`terse` 那 4 条是同一句话配四个订单号）。**两份最会被外人读到的文档，
把头条读数的证据强度报大了三倍，方向还都对作者有利。**
已在 README ×2、`READING_THE_NUMBERS.md`、`GENERALIZATION_FIX.md` 全部改为
「60 条任务只用到 35 条不同措辞，去重后最小格 n=1」。

### LOG-20260819-01：针对性数据修复在同源评测面上改善、在分布外退化——**按事先规则判负，候选不变**

- 日期：2026-08-19
- 阶段/任务：R7 质量收口
- 状态：解决（**改变后续验证一次数据修复的做法**，并改变对切分的认识）
- 关联：LOG-20260816-01（表面形式 → 动作）、LOG-20260817-07（dev 不能替代封存集）、ADR 0005

**发生了什么**：项目最大的失败模式（该拒绝却执行）此前**在方法上无从下手**——
dev 60 与 `ood_dev` 60 两个可迭代面在候选上都已饱和（政策违规 0–2 与 0），
唯一看得见它的是只能少量观测、且结果不得反馈进开发的封存集。

**诊断（只用公开生成器代码，零模型读数）**：冻结数据集按 `sha256(family)` 切
20/5/10，**不对难度分层**，而拒绝类场景的难度就是 `margin` 离边界多远。结果是
`refund_denied_window` 的训练集里 margin ≥ 10 的 family 只占 **10%**、封存集占 **50%**
——**5 倍的训练/测试分布偏移，发生在一个被认为已冻结、干净的切分内部**。
`refund_recovery` 同样是 4 倍。**全部封存政策违规恰好落在偏移最大的那个场景。**
这把「dev 不能替代封存集」从一次事后观察变成了切分本身的可测量性质。

**仪器**：政策边界探针沿 `refund_deadline − current_day` 扫 15 个偏移量 × 8 条，
复用整条 OOD 评测路径（`kind_success` 直接是决策曲线），真值逐点比对可执行政策规则。
实测为**确定性仪器**：同一权重换 commit 重跑，逐点与总分逐位相同。

**诊断读数**：`sft-008` 在 14/15 个偏移量上是 1.00，**唯独 `offset −14` 塌到 0.375**，
全探针 5 次违规都在那一点。零训练基座在该点是 1.00——**但那不是合规，是不动手**：
它在放行侧只有 0.00–0.38，56 次失败全是 `premature_final_response`。
**一个只测拒绝侧的评测集会把零训练基座判成完美遵守政策。**

**修复与判定**：在冻结网格**之外**的 margin 上补训练素材（+208 行，两侧对称，
与探针评测点零交集，隔离由代码断言）。`sft-009` 的读数：

| 评测面的措辞来源 | 方向 |
|---|---|
| 探针（与冻结集**同源**的两条模板） | 改善 0.9583 → **0.9750**，违规 5 → **2** |
| dev 60（冻结集自己的 12 句模板） | 改善 58/60 → **59/60** |
| `ood_dev` 60（LLM 措辞池，**分布外**） | **退化 0.9833 → 0.9500**，被干预的那个场景 0.90 → **0.70** |

事先写定的规则（提交于 `3cb5619`，早于全部修复运行）要求「修好」三条同时成立，
实测**三条全不成立**；「修坏」的触发条件两项**各自独立成立**。
**因此不更换发布候选，`sft-008` 保持不变。**

规则是严格的：只看探针总分、违规数与 dev，`sft-009` 是更好的模型。
规则判它负，是因为规则要求"不许拿分布外表现换分布内表现"。**规则不事后放宽。**

**一般化的教训（这条改变后续做法）**：

> **只用与训练素材同源的评测集验证一次数据修复，会看到一个不存在于分布外的改进。**
> 方向会随措辞分布翻转，而且退化可能恰好落在被干预的那个场景上。
> 因此：**任何数据侧修复的验收，必须同时给出一个措辞（或其他表面形式）分布外的读数**，
> 且分布外退化优先于分布内改善。

推论两条：

- **切分要按难度分层，或者至少把难度分布测出来写在旁边。** 一个"随机哈希切分"
  会在冻结契约内部造成 5 倍的训练/测试分布偏移，而没有任何机制会提醒。
  本项目的冻结契约不能改，因此这条以「测出来并披露」落地
  （`test_the_frozen_split_does_not_stratify_by_difficulty`）。
- **诊断集的两侧都要在场。** 只测拒绝侧会把"从不动手"判成"完美合规"；
  只测放行侧会把"见谁都退款"判成完美。探针两侧共用同一组措辞并有测试断言，
  否则模型能靠语气而不是靠状态作答。

**同轮的一个独立观察**：teacher（DeepSeek，temperature=0）为增强素材采集 56 条轨迹时，
**52 条被接受、4 条判 `policy_violation`，而那 4 条全部落在最极端的超期格**
（margin 12/16/18）。**一个规模大得多的商业模型在同一个状态区域上违反同一条规则。**
这说明该区域的困难不是小模型微调的怪癖。**但只有一个 provider、一个 prompt、
一次采集**，不足以支撑"这是通用现象"这种说法。

**同轮事后自查发现的一处实验设计缺陷（写进来是因为它改变后续做法）**：
增强素材两侧共用同一组请求模板。这条设计对**探针**是对的（两侧措辞不同的话，
模型能靠语气而不是靠日期作答）；**但把它用到训练数据上是另一件事**——
那两条模板在冻结训练集里**只与"拒绝"结果同现**，于是本轮给它们补了约 112 行
"放行"样本，**主动打断了一个既有的「措辞 → 结果」关联**。
**这次干预因此不是单变量的**：状态覆盖（有意）、措辞→结果映射（无意）、
行数与梯度步数（已披露）三者同时变了。第二条与"分布外退化恰好落在
`refund_denied_window`"方向一致，但本轮没有能分离三者的实验，不写成因果。

> **推论：训练增强的 user 请求必须取自被增强场景自己的模板。**
> "两侧共用措辞"是**评测集**的正确设计（防止靠语气作答），
> 把它照搬到**训练集**上会额外改写措辞与结果的关联——两者的正确设计相反。
...
### LOG-20260822-05：R9 Phase B 第三轮 + OOD v4 跨工具评测集——A/C 验证结果

- 日期：2026-08-22
- 阶段/任务：R9 / Phase B 第三轮候选（sft-003）+ 跨工具评测集建设
- 状态：观察（A 的假设被证伪；C 完成并给出 Phase B 核心读数）
- 关联：LOG-20260822-04

**做法**：
- **A**：去掉 refund_then_cancel 的 oversample（唯一变量），重导训练集
  （1920 行，`train-export-v4-003`），重训 `sft-003`
  （eval_loss 0.107，三轮最低）。
- **C**：新建 OOD v4 跨工具任务集——12 场景 × 10 = 120 条，v4 bundle，
  措辞取 bank-v4 的 `ood_dev` 分片（与 train_aug 互斥）。为支持双订单场景，
  扩展了占位符体系与 `OodDatasetVersion` 字面量（新模块
  `domain/ood_v4_tasks.py`，登记版本 `retail_ops_ood_v4_20260823`）。
  这是 **Phase B 核心主张（语义重叠工具选择）的第一个正面计量口径**。

**sft-003 三面读数**：

| 评测面 | sft-001 | sft-002 | **sft-003** |
|---|---|---|---|
| v4 dev（120） | 0.95 / pv 0 | 0.9167 / pv 0 | 0.9167 / pv 0 |
| OOD v2（v1 bundle，60） | 0.8167 / pv 0 | 0.8333 / pv 8 | **0.8667** / pv 7 |
| OOD v4（跨工具，120） | — | — | **0.8917** / pv 2 |

OOD v4 逐场景：11/12 在 0.8–1.0（cancel_eligible/cancel_recovery/
check_refund_status 均 1.0——**单订单的工具选择已学会**）；
唯独 refund_then_cancel **0/10**。

**A 的假设被证伪**：去掉 oversample 后 rtc 仍是 dev 0/10、OOD v4 0/10。
结合三轮数据重新归因：rtc 崩坏与 **paraphrase 增强本身相关**
（无增强的 sft-001 是 4/10，有增强的两轮都是 0/10），不是 oversample。
机制猜测（未验证，不写成结论）：改写放大了「提到的订单 → 用主导动作处理」
的先验，而退款动作在语料中的频率远高于取消。

**诚实结论**：
1. **Phase B 核心读数成立但有限定**：5 工具面上 11/12 场景的跨工具选择
   （含 get_refund_status vs get_order、cancel_order 单订单场景）达到 0.8–1.0；
   失败集中在**双订单复合动作**（对第二笔订单执行 cancel）这一个点上。
2. **sft-003 是当前最优候选**：OOD v2 三轮最高（0.8667），pv 中等（7），
   且首次有了跨工具正面读数。候选替换与否留给用户决策（当前发布候选仍是 sft-008，
   其口径不同——3 工具、960 行、含 sealed 观测——不可直接比较）。
3. duplicate-deny 的执行倾向代价仍在（pv 7），与 R6 记录的同构代价一致，
   未做 B 方向对冲（遵守本轮单变量纪律）。

**备选方案与未选择理由**：未做 rtc 的 Oracle-only 统一序实验——A 已证明问题
不在调用顺序混合；下一轮若继续，方向应是「提高 cancel 动作先验」（如增加
cancel 类场景配比或对 RTC 构造中间辅助任务），而不是继续调这个场景的采样权重。

**后果与下一步**：Phase B 判读按 R9_SPEC §3.3：OOD v2 0.8667 < 0.80 目标线？
...
### LOG-20260827-01：R10 退化曲线的自变量从未变化，结论作废；v3 评测集三处不自洽已修

- 日期：2026-08-27
- 阶段/任务：R10 更正（不改 R10 已完成状态，按「已完成阶段只能追加更正记录」处理）
- 状态：已确认（纯 CPU 证据，未消耗 GPU / API / 封存 holdout 观测）
- 关联：LOG-20260824-01（被本条更正）、commits `8e43317` / `97ff796` / `88ccabb`

**背景与难点**：例行核查 08-25 至 08-27 三次未入台账的提交时，用 Oracle（gold
`expected_calls`）在 v3 dev 上做自洽性回放——这个检查此前不存在，v3 的测试只断言
配额、确定性与 ID 唯一，不断言「正确答案在环境里真的走得通」。

**证据**：

1. **`tool_count` 是空转参数**。`build_toolcount_task_set` 在 5 个断点上产出的
   train/dev 任务**逐条 content_sha256 相同**；`scripts/run_v3_degradation.py::_make_env`
   又对每个断点加载整份 15 工具 v3 bundle。因此 {3,6,9,12,15} 五轮训练与评测
   看到的是同一批任务和同样的 15 个工具。**曲线平坦是恒真，不是读数**，
   LOG-20260824-01 第 3 条与 R8 C2「N=6/9/12/15 四点全部 0.45/0.70」
   （四点读数逐位相同，本身就是该缺陷的指纹）**均不构成工具数效应的证据**。

2. **评测集自身不自洽，且 08-26/08-27 的改动让它更不自洽**。Oracle 回放 v3 dev：
   `4b2044e`（改动前）110/120，`88ccabb`（HEAD）**90/120**。
   - `8e43317` 把 `REFUND_THEN_CANCEL` 的 gold 从「退 A + 取消 B」改成「取消 A」，
     场景名与 `target_state`（仍要求 `refund_status=refunded`）都没跟着改，
     该场景变成永远不可达；R9 Phase B 记录的「双订单复合动作硬失败」**不受影响**
     ——那条结论跑在 v4 任务集上，v4 的 Oracle 回放为 120/120，双订单真实存在。
   - `97ff796` 引入 `metadata["skip_reads_gate"]`，让任务数据能关掉
     `cancel_requires_lookup` 这条政策守卫；条件写成 `len(expected_calls)==1`，
     而 `CANCEL_RECOVERY` 有两次调用，于是它的 gold 序列自己触发 10 次政策违规。
   - 同一提交声称「base 0.31 → sft-008 0.58」，该读数产生于上述已损坏的评测集，
     且只存在于 commit message，无台账、无产物目录。**作废**。

**决定与方案**：

- 政策守卫**只由代码强制**：撤掉 `skip_reads_gate`，并加回归测试断言任务
  metadata 无法关掉这道门。评测任务是数据，数据能关政策，政策就不再是判据。
- 断点必须真的改变模型看到的工具：`RetailOpsEnv` 新增可选 `allowed_tools`
  （默认 `None` = 全部，v1/v2 已产出证据逐字节不变）；`ToolCountTaskSet.tool_names`
  给出该断点的工具集；采集与评测两条路径都改为按它构造环境。
- 所需工具没被呈现的场景在该断点上无解，因此排除（`scenarios_for`）。
  代价是**总体 task_success 不可跨断点比较**，曲线只能读 `common_scenarios()`
  ——恰好是 v1 的 6 类，因此 {3} 断点复用 `sft-008` 作左端点仍然成立。
- 新增两类此前缺失的测试：Oracle 必须解出每个断点的全部 dev 任务且零政策违规；
  断点必须真的限制 `env.list_tools()`。修复后 5 个断点 Oracle 均 1.0000 / 0 违规。

**备选方案与未选择理由**：另一条路是固定「必需工具 4 个 + 干扰工具数变化」，
任务集跨断点完全一致、总体 task_success 可比。未选，因为它让 {3} 断点不存在，
`sft-008` 复用作左端点这个已定设计随之失效；本轮修的是「自变量没生效」，
不重新设计实验。

**后果与下一步**：R10 的退化曲线读数与 R8 C2 的四点读数**都不再可引用**；
要拿到真读数必须在修复后的代码上重跑（GPU，需单独授权）。`sft-008` 作为发布
候选不受影响——它的全部读数来自 v1/v2 口径，与 v3 无关。EXECUTION_PLAN 的 R10
节已追加更正记录。

### LOG-20260904-01：v1.3 发布门禁语义上线——绝对违规下界 + 最小效应宽度

- 日期：2026-09-04
- 阶段/任务：质量收口第二轮 Phase B（交接 `2026-09-04-quality-closeout`）
- 状态：代码与测试已上线并全绿；**阈值（0 / +0.02）已于 2026-09-04 由用户确认冻结**
  （LOG-20260904-02，见下）；尚未用它做任何发布判定
- 关联：PITFALLS #19（相对门禁的盲区）、HOLDOUT_LEDGER 观测 5/6 的 GO、
  `docs/handoffs/2026-09-04-quality-closeout-v13-and-audit-execution-prompt.md`

**方法论事件**：发布门禁从「相对比较 + OOD 绝对下限」演进为「相对比较 + 绝对安全门 +
最小效应宽度」。新增 `GATE_IDS_V1_3 = (*GATE_IDS_V1_2, "policy_violation_count_max",
"success_delta_ci_lower_min")`。动机是 PITFALLS #19 记录的两处盲区：违规 11→7 能过
相对门但 7 次违规照样 GO；`success_delta_ci_lower` +0.0083 贴 0 过门，统计上无法拒绝
零差异。

**实现纪律**：加门不删门——v1.0/v1.1/v1.2 的 `GATE_IDS` 逐字节不动（既有测试锁定），
17 份磁盘 release 报告与 13 份 sealed 证据在新代码上加载与 report_id 复算不变（回归
测试）。阈值钉在 `ReleasePolicyConfig` 的 `Literal[0]` 与精确相等 validator（float 不能
进 Literal），`release.yaml` 文件一个字节未动——它在 `bundle_sha256` 的分量里，既有
证据的配对前提依赖该哈希不变。

**诊断性重算（不是发布判定）**：sft-008 合并形态在 v1.3 口径下观测 5/6 都 NO-GO
（违规 2/7 > 0；+0.0083 < +0.02）。落盘于 `reports/retail_ops/v1/r6/v13-diagnostic-*`，
历史 GO 原样保留、两套判定并列。**这次重算只复算既有证据，不消耗封存观测**；真正的
v1.3 发布判定按交接在 Phase D4 一次性进行，判读规则届时先写定并提交。

**备选方案与未选择理由**：把阈值写进 `release.yaml` 被否——会改变 bundle_sha256，使
全部既有 dev/sealed 证据不可配对；把阈值写成普通常量被否——无锁，改值无红测试。Literal/
validator 锁与 `invalid_call_count_max: Literal[0]` 的既有模式同构。

**后果与下一步**：新发布可配置 v1.3（CLI `_load_ood_evidence` 已接受 1.2/1.3；CLI 配对
守卫从 `GATE_IDS_BY_SCHEMA` 结构化推导，新口径自动覆盖）。下一步：用户确认阈值数值 →
D4 一次性 v1.3 发布判定（新措辞池封存 + 全量读数）。

### LOG-20260904-02：v1.3 两个阈值数值冻结（0 / +0.02）

- 日期：2026-09-04
- 阶段/任务：质量收口第二轮 Phase B 收尾（决策门 #1）
- 状态：已冻结（用户确认）；D4 一次性发布判定的前置条件满足
- 关联：LOG-20260904-01、B3 诊断性重算

**决定**：`policy_violation_count_max = 0`、`success_delta_ci_lower_min = +0.02` 按提案值
冻结，不再调整。依据：B3 诊断已验证两门在观测 5/6 真实证据上的判别行为（拦下违规 2/7
与 +0.0083）；+0.02 与 n=120 配对 bootstrap 的可分辨宽度一致（贴 0 的 +0.0083 无法与
零差异区分，+0.02 起才具备最小效应意义）。

**后果与下一步**：v1.3 口径可作为 D4 一次性发布判定的判读口径；判读规则全文在 D4 前
写定并提交。DPO（决策门 #4）用户裁定：**D4 之后再说**。宽工具面迁移（决策门 #6）用户
裁定：**不迁移**。GPU 任务（C2/D1/E2/D4）在 gpu-5090 上执行，命令清单见下一份交接。

### LOG-20260905-01：rtc 双订单复合动作硬失败被辅助任务课程修复（甲判「方向对」，乙判「修好」）

- 日期：2026-09-05
- 阶段/任务：R9 Phase B 第四轮（交接 `2026-08-23-r9-phase-b-round4` + 本阶段交接 §4 第 2 步）
- 状态：完成（探索性结论；不用于发布判定，发布候选仍是 sft-008）
- 关联：`R9_PHASE_B_RESULTS.md` §8、PITFALLS #9、判读规则预注册（round4 交接；
  task_plan `3db975f`/`7472c2c`）

**方法论事件**：rtc（refund_then_cancel）双订单复合动作在三轮全败（全评测面 0/10）。
第四轮按预注册两步走：方案甲（cancel 类 family 20→35，`retail_ops_v4_20260904`，
teacher 换 mimo-v2.5）改善到 dev 4/10、OOD v4 5/10——**判读落第二分支「方向对，
力度不够」**；方案乙（新增 rtc_stepwise 辅助场景——RTC 同一状态拆两段的第一段
「查 B 并取消 B」，与完整 RTC 1:1 混入训练，`retail_ops_v4_20260905`，sft-005）把
rtc 修到 dev **9/10**、OOD v4 **8/10**，三面全过预注册线 → **修好**。判读表阈值
一个字未改，观测发生在预注册提交之后。

**证据**：v4 dev 0.9833/pv1（dev face 为 20260905 重抽，与旧轮不可逐位比）、
OOD v2 **1.0000**/pv0（任务集与前三轮逐字节相同；invalid 1，schema 0.9899）、
OOD v4 0.9583/pv3（pv 较甲 2→3，如实记录）。teacher 接受率甲 0.987 / 乙 0.980
（mimo-v2.5，与三轮 DeepSeek 的 provider 差异如实记录；甲乙内部单 teacher 无混采）。

**与已证伪方向的关系**：调 rtc oversample 权重（三轮证伪：×3 恶化到 0/10）与
统一调用顺序（三轮证伪）均未复活；本轮新增的实证是——**「提高 cancel 类 family
覆盖」（甲，4/10）不足以修复，只有把复合动作拆成第一段辅助任务单独训练（乙，9/10）
才修复**。机制解读（未单独消融）：辅助任务给了「第二订单的请求 → cancel」的干净
梯度，完整 RTC 的梯度被「提到的订单 → 主导动作（退款）」的强先验淹没。

**备选方案与未选择理由**：加大 cancel 配比（oversample）被用户在甲/乙选择中未选——
它有 PITFALLS #9 的反证且机制较弱；方案甲单独继续加大覆盖被「family 池 10 态已满」
限制，转乙是 round4 预注册的第二分支指定路径。

**后果与下一步**：Phase B 探索性收口（第四轮完成）。三条边界如实记录：OOD v4 pv
...
### LOG-20260905-03：D4 一次性 v1.3 发布判定——NO-GO（绝对门首次真实拦下发布候选）

- 日期：2026-09-06
- 阶段/任务：质量收口第二轮 Phase D4（交接 `2026-09-04-gpu-phase-c2-d1-e2-d4` §4 第 4 步）
- 状态：完成。判定 NO-GO；不重跑、不换素材；结果不反馈进任何后续开发
- 关联：LOG-20260904-01/02（v1.3 上线与冻结）、HOLDOUT_LEDGER 观测 7、
  OOD_SEALED_LEDGER 观测 3（v2.3）、task_plan 预注册 `9b1c61b`

**方法论事件**：v1.3 十二门的**首次真实发布判定**（此前 B3 只复算既有证据）。
运行内容与判读规则先提交后观测（代码冻结 `656ce20` 之后运行）。发布候选
`sft-008`（原始 seed 0 合并形态，远端 merged 模型缺失后由
`scripts/ops/merge_lora_adapter.py` 现场重建——`model.safetensors` SHA-256 与
观测 5 配置声明逐位一致，确定性合并）。结果：11/12 门 PASS（含最小效应宽度门
+0.0583 ≥ +0.02 首次真实通过、OOD v2.3 两门 PASS），唯一 FAIL 是绝对门
`policy_violation_count_max=0`（观测 2 > 0）→ **NO-GO**。同一候选在 v1.0/v1.1
口径下会再次 GO（读数与观测 5 逐位一致：117/120、违规 2、ci_lower +0.0583）——
**绝对安全门按设计意图拦下它**：宣告当前候选在「封存集零政策违规」线下不合格，
根治路径是 DPO（用户已裁定 D4 之后启动）。

**证据**：封存 holdout 观测 7（base 0.8583/11 违规、cand 0.9750/2 违规、
per_call 1.112）；OOD v2.3（第四份素材 bank-004/mimo：base 0.6167、cand 0.9833、
delta +0.3667——四份素材上候选 1.0000/0.9833/0.9833/0.9833，多素材鲁棒再次成立）。
互斥性实测全 0（进 Git 清单）。素材生成条件差异如实记录（mimo 替代 DeepSeek、
retry brief 强化、per_intent 150——前两次生成失败于分片覆盖断言）。

**备选方案与未选择理由**：先修违规再判定（DPO 在 D4 前）被用户裁定否——D4 的
目的就是记录当前候选在绝对门下的真实判定；换素材再试被预注册纪律禁止。

**后果与下一步**：阶段收口（质量收口第二轮 C2/D1/E2/D4 全部完成）。判定按新口径
成对陈述（NO-GO 必须与 11 门 PASS 及 OOD 读数成对出现）。DPO 为下一阶段的
用户决策门。

### LOG-20260906-01：DPO 主线一轮完成——判读修坏，门禁守卫按设计拦下「平移不是校准」的 DPO 版

- 日期：2026-09-06
- 阶段/任务：r11 DPO 主线（交接 `2026-09-06-dpo-and-parallel-execution-prompt` 轨道 A；
  预注册 `eeec309` + 两次修订 `5ee0fa3`/`8bc2dd2` 均先于观测提交）
- 状态：完成。判定 **修坏**：不换候选（sft-008 保持发布候选）；记录负结果；
  不重跑、不换采样素材再试。**D4 的 NO-GO 状态维持**（绝对门违规 2 无新解）。
- 关联：`DPO_ENTRY_EVIDENCE_D2.md`（设计宪法）、`POLICY_BOUNDARY.md` §6（R7 平移）、
  task_plan DPO 预注册、findings 2026-09-06 两条读数

**方法论事件**：DPO 入口四条件此前全部满足（D2），本轮以门禁守卫方案
（DENY-only 偏好对 + 预注册「放行侧不塌」门禁）执行一轮完整闭环：真实链路采样
（240 任务 × N=8，27 对全部来自 `offset −14` 的模型自身失败采样，放行侧执行类
违规 0）→ QLoRA-DPO（27 对 × 3 epochs = 12 步，loss 0.6931→0.6712、守卫通过）→
三面评测。判读（A-6 冻结规则逐字执行）：**修坏**——探针 −14 被校准到 1.00 的
同时放行侧 8 点全部 < 0.90（8 点全 ≤ 0.25，premature_final_response 58 条）、
dev 0.8833、ood_dev 0.6333 双双退化。「修好」三要件全部不成立，「没动」分支
排除（信号方向有效、强度足以移动边界）。**单一方向的偏好信号把「拒绝退款」
泛化到放行侧——探针横轴直接画出边界整体平移的形状；预注册的门禁守卫正是为
这个失败模式存在，它按设计拦下了本次候选。**

**备选方案与未选择理由**：高温诱导对冲对（用户裁定否——rejected 质量与配对噪声
风险）；epochs 维持 1（用户裁定修订 1→3——27 对实际产出 vs 预注册时 100–200 对
假设的功效差，修订发生在任何观测之前）；对冲方向偏好对未试（预注册纪律：判读
后不重跑不换素材再试）。

**后果与下一步**：DPO（本设计）被证伪并入账；**不能**一般化为「DPO 方法对此
失败模式无效」（n=1、无消融、对冲方向未试）。绝对门（封存集零违规）的根治
路径回到待定；剩余已备料入口 = **B-4 合并数据重建**
（`PROPOSAL_DATA_REBUILD_B4.md`：分层切分 + max_steps 6 + reason 口径 A，
三个决策门待用户裁决）。sft-008-dpo-001 不进入任何候选比较或对外材料；
D4 判定（NO-GO）的成对陈述口径不变。

### LOG-20260907-01：B-4/v5 全链一轮完成——封存观测 8 判定 NO-GO（11/12），绝对门历史首过，唯一失败门 invalid_call_count=2

- 日期：2026-09-07
- 阶段/任务：B-4 数据重建（v5）轨道 A 全链（预注册 `ba463a6` + 修订 `b290b16` 均先于观测提交）
- 状态：完成。判定 **NO-GO（11/12 门 PASS）**：候选不换（sft-008 保持）；按 A-6 第三
  分支「（或反之）」升级，用户裁定（2026-09-07）如实收官。**绝对门
  `policy_violation_count_max=0` 历史首次 PASS**。
- 关联：`PROPOSAL_DATA_REBUILD_B4.md`、`POLICY_BOUNDARY.md` §2（根因 1）、
  HOLDOUT_LEDGER 观测 8、findings 2026-09-07 各节

**方法论事件**：B-4 的立项主张——「5.0× 训练/封存难度偏移是政策违规的结构性根因
（根因 1）」——被一轮完整闭环证实：难度分层重建（margin 档 = 分层键、margin 0 档
进三分、口径 A、max_steps 7）后，封存 246 条上政策违规 81（base）→ **0**（candidate），
且 dev（1.0000/pv0）、探针（15/15 = 1.00，−14 修复且放行侧完好）、OOD v2（1.0000）、
OOD v4（0.9917）四面全部零违规。**11/12 门 PASS**：delta +0.4309、ci_lower +0.3699
（最小效应宽度门以 18 倍余量通过——统计强度问题同步解决）、三个延迟/步数比值全部
≤1.25（合并形态）、OOD 两门 1.0000/+0.3833。唯一 FAIL：`invalid_call_count = 2`
（两次空生成 parse 滑步，均在成功任务内恢复；非工具拒绝、非政策违规；2/246 = 0.8%）。

**预注册分支的形状缺口与处置**：A-6 三分支预设「修好=12/12」「修坏=绝对门 FAIL」，
未预见「绝对门修复但另一门 2 次滑步」的组合；第三分支「边界改善但未达标（或反之）」
按「升级需用户确认」条款升级，用户裁定如实收官 NO-GO（机械字面的「修坏→绝对门路径
就此穷尽」与事实相反，不采用）。**负结果入账的是 2 次滑步这一新失败类，不是绝对门
路径本身**。

**备选方案与未选择理由**：修请求重冻结（新 dataset_version 作废全部已采证据，为
0.6% 的行付全价——用户否）；rtc_stepwise 0.643 < 0.80 分桶门按现状放行（同上，
用户裁定）；重跑观测求不同 parse 运气（向冻结集掷骰子，纪律禁止）。

**后果与下一步**：候选不变（sft-008）；空生成毛刺（2/246）与 rtc_stepwise 请求
缺陷（继承自 v4）列为已诊断的迭代入口，需新预注册 + 全价。v1 口径（观测 1–7）
终止，v5 口径自观测 8 起（台账已标注）。

### LOG-20260908-01：V6 迭代全链一轮完成——观测 9 判定 NO-GO（10/12），解析器容忍与 rtc 修复双双门级验证，探针/OOD 拦下措辞 basin 塌方

- 日期：2026-09-08
- 阶段/任务：V6 迭代（预注册 `696ad81` + 修订 `5810959`，先于一切运行；A-0 四项
  用户裁定 + 缺标签处置裁定解析器容忍）
- 状态：完成。判定 **NO-GO（10/12 门 PASS）**：候选不换（`sft-008` 保持）。
- 关联：HOLDOUT_LEDGER 观测 9、findings 2026-09-08 节、`docs/PITFALLS.md` §二 #27

**方法论事件 1——诊断修正与处置（V6-1）**：观测 8 的「空生成」分类被证伪——实录是
「有效 JSON 缺 `\n</tool_call>` 闭合标签」（tokenizer 边界退化，非空生成；记录层
`raw_text` 字段恒 null 造成观察者误读）。处置经用户裁定为**解析器协议容忍**（评测
语义变更首用版本化机制：新 `parser_id` + manifest 单源钉定 + OOD config 键 +
发布侧一致性核对 + None 不参与内容哈希保老证据逐位复算）。**门级验证**：观测 9
`invalid_call_count=0`（v5 唯一失败门归零），`unterminated_call_count` 诊断计数
与报告同盘可查。

**方法论事件 2——rtc 修复采集端验证**：请求 reason 与 gold 同源的最小修复使
rtc_stepwise 分桶接受率 0.643 → **1.000**，总体 96.3% → **98.5%**；采集前枚举域
断言（PITFALLS #26 教训前移）上线并被语料验证（v5/v4_0905 缺陷形状当场拒绝）。

**方法论事件 3——探针/OOD 拦下措辞 basin 塌方（本轮核心负结果）**：v6 candidate
在同源词域近乎满分（dev 0.9949/pv1、封存 0.9959/pv1）而在 v1 风格枚举词域塌方
（探针 DENY 侧 7 点 0.00–0.107、OOD v2 0.6833 < 0.70 绝对门 FAIL）。根因链：
枚举词 deny 请求在 v5/v6 训练分布中都不存在（口径 A 移除枚举词）→ 对该词域的
拒绝泛化是训练运行的 basin 性质（数据 diff 逐项排除后收敛；R8 seed 方差 7 倍跨度
的定性版）；v5 的 15/15 是 basin 运气而非被训练出的能力。**「同源评测面高估」
第三次实证**（R7、R11 之后）；预注册探针条件与 OOD 绝对门独立于封存读数拦下候选
——预注册纪律第四次实证。教训：措辞鲁棒性需要显式词域覆盖 + 双词域判读。

**后果与下一步**：候选保持 `sft-008`；不重跑、不换素材。词域覆盖（枚举词 deny
请求入训练 + 双词域判读）列为下一迭代候选方向，待单独预注册与用户决策。
测试基线 1544（观测计数检测器形状网补强 + 两处同型豁免，无新增测试函数）。

### LOG-20260908-02：v7 词域覆盖迭代一轮完成——观测 10 判定 NO-GO（11/12），OOD 绝对门历史首过，「枚举词 deny 缺失」根因链被预测-验证闭环证实，远端塌方与封存边界摆动并案收官

- 日期：2026-09-08
- 阶段/任务：V7 迭代（预注册 `bc1fb01`，先于一切运行；A-0 两方案点用户确认：
  只动 train_export + 2 模板变体/任务）
- 状态：完成。判定 **NO-GO（11/12 门 PASS + 预注册探针条件独立 FAIL）**：候选不换
  （`sft-008` 保持）。
- 关联：HOLDOUT_LEDGER 观测 10、findings 2026-09-08 V7 节、`docs/PITFALLS.md` §二 #27

**方法论事件 1——根因链的预测-验证闭环**：v6 收口时判定「枚举词 deny 请求在训练
分布中不存在 → 对该词域的拒绝泛化是 basin 性质」（#27）。v7 把枚举词请求显式放进
训练分布（426 行，deny 三场景 + allow 三场景平衡配额，实例只来自 train split），
预测「该词域的拒绝行为不再依赖 basin 运气」。实测：OOD v2 绝对门 0.6833 → **0.8333
（历史首过 ≥0.70）**、探针近边界（−1..−5）0.00–0.107 → **全部 ≥0.875**、探针
pv 50 → 14，且同源面（dev 0.9949/pv1）与探针 ALLOW 侧（8 点全 1.00）零损伤——
**干预在其靶面上按设计生效，根因链被证实**。#27 的「必须显式词域覆盖」从假设
升级为被验证的工程结论。

**方法论事件 2——覆盖泛化的分级性（新边界事实）**：同一干预下探针远端
（−7 = 0.250、−10 = 0.500、−14 = 0.625）仍塌——**近边界修复 ≠ 远端修复**。
训练侧枚举词行在各 deny margin 档密度大致均匀，「档位密度不足」这一候选解释被
排除；远端机制未定，不得在无诊断的情况下立项修复（预注册纪律：不为数字好看
重掷骰子）。

**方法论事件 3——go/no-go 与绝对门的抽签成分（预注册风险 1 的实测）**：封存
candidate 0.9797/pv5/inv0（绝对门 FAIL）——5 次违规 = 4× `cancel_denied_recent`
（上轮为 0 的场景族，重训后 deny 边界摆动到新位置）+ 1× 观测 9 同款
`refund_denied_window` variant-1。两轮重训失败点不固定，而绝对门对任何摆动
零容忍：**「修好已诊断失败面」与「封存面零违规」是两个独立命题**，后者含不可
消除的运行方差。预注册如实记录，判 NO-GO 不代表干预失败。

**后果与下一步**：候选保持 `sft-008`；不重跑、不换素材。远端塌方机制诊断与
封存边界摆动的方差治理是下一迭代的两条候选入口，均需单独预注册与用户决策。
测试基线 1569（新增枚举词导出测试 25 条，四文档基线同步）。

...
> 注：`PROJECT_LOG` / `progress` / `CAREER_CONTEXT` 自 2026-09-08 起为作者本地工作文档，
> 不入库；条目号保留供作者本地追溯。
...
> [`MODEL_CARD_sft-006.md`](./MODEL_CARD_sft-006.md)（观测 5 起主角为 `sft-008`，
> 最终判定见 [`HOLDOUT_LEDGER.md`](./HOLDOUT_LEDGER.md) 观测 7–10）。
> **观测次数与判定的唯一事实源是
...
3. **延迟精度有限**：gpu-5090 为多人共用，评测期间他人占用在 12.6→11.9→10.8 GB、
   ...
   但该数**不得表述为精确测量**。
4. **holdout 的观测次数与判定以 [`HOLDOUT_LEDGER.md`](./HOLDOUT_LEDGER.md) 为唯一事实源**：
   结果不得反馈进开发、调参、prompt/parser 或 checkpoint 选择。
   任何新判定都要**再消耗一次**封存 holdout 观测，需用户单独决策。

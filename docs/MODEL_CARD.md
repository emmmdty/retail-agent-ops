...
### 4.2 封存 holdout（120 条，仅用于发布判定，2026-08-11 首次也是唯一一次观测）
...
**根因已定位**：240 条训练数据中 160 条（66.7%）只含 1 次工具调用，模型学到
"调一次 → 写总结"的主导模式并过度泛化。旁证：`average_tool_calls` 1.25→1.18、
`average_turns` 2.25→2.09、`average_output_tokens` 109→147（学到 teacher 的详尽风格，
也解释了平均延迟从 2388 ms 升到 4530 ms）。该机制在 dev（60 条）与 holdout（120 条）
上一致复现，不是小样本偶然。
...
3. **延迟精度有限**：gpu-5090 为多人共用，评测期间他人占用在 12.6→11.9→10.8 GB、
   ...
   但该数**不得表述为精确测量**。
4. **holdout 已被观测一次**：其结果不得反馈进开发、调参、prompt/parser 或 checkpoint 选择。
   改进后的候选若要再次判定，需另行决定是否消耗第二次观测。
5. **n=120 的统计功效有限**：±7.5pp 量级的 CI 宽度决定了本项目无法分辨小幅差异，
   这是任务集规模的固有限制，不是评测实现的缺陷。
6. **领域窄**：2 个业务工具、6 类任务、单一中文零售退款场景。跨领域泛化未做任何验证。

## 8. 后续方向（R4 输入）

失败类别数量足够、可观测、可复现，满足 R4 的改进准入条件。首选方向是**训练数据的动作
长度分布**（当前 66.7% 单次调用），而不是更换算法。是否进入偏好优化需等 SFT 路线确认停滞，
且必须先有足量执行有效的偏好对。

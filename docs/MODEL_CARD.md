...
> 封存 holdout 已消耗 **2 次**观测；观测 2（2026-08-14）的主角是另一个候选
> `sft-006`，见 [`MODEL_CARD_sft-006.md`](./MODEL_CARD_sft-006.md)。观测次数与
> 判定的唯一事实源是 [`HOLDOUT_LEDGER.md`](./HOLDOUT_LEDGER.md)。
...
3. **延迟精度有限**：gpu-5090 为多人共用，评测期间他人占用在 12.6→11.9→10.8 GB、
   ...
   但该数**不得表述为精确测量**。
4. **holdout 的两次观测均已消耗**（2026-08-11、2026-08-14）：结果不得反馈进开发、调参、

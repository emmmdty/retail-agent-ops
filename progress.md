...
## 2026-09-06 — GPU 执行阶段第 4 步：D4 一次性 v1.3 发布判定——NO-GO（绝对门生效）

- 预注册（`9b1c61b`）→ bank-004 生成（mimo，932 条，retry brief 强化后第三次成功；
  前两次失败于分片覆盖断言）→ 互斥实测全 0 → 版本登记 v2_3（TDD）→ sealed 分片
  构建（60 任务）→ merged 模型现场重建（逐位复现 `70981220…`）→ 一次性执行
  （OOD×2 + holdout×2 + release v1.3）→ 判定 **NO-GO**。
- 12 门：11 PASS + **`policy_violation_count_max` FAIL（观测 2 > 0）**。候选在
  v1.0/v1.1 口径下会 GO（读数与观测 5 逐位一致），v1.3 绝对安全门拦下——首次
  真实发布判定按设计意图产出诚实结果。OOD v2.3 候选 0.9833 / base 0.6167。
- 台账：HOLDOUT_LEDGER 观测 7、OOD_SEALED_LEDGER 观测 3（v2.3 退役）、
  EXECUTION_PLAN、LOG-20260905-03。结果不反馈进任何后续开发；DPO 待用户启动。

| Date | Command | Result |
|---|---|---|
| 2026-09-05 | bank-004 生成 ×3（mimo） | 932 条；brief 强化后过分片断言；~1.5M tokens（记账） |
| 2026-09-06 | 互斥实测 | bank-004/ood_sealed 226 条与全部分片及训练集交集 0 |
| 2026-09-06 | merged 模型重建 | model.safetensors `70981220…` 与观测 5 逐位一致 |
...
## 2026-09-06 — GPU 会话 2/3：R11-2 训练 + R11-3/4/5 三面评测 → DPO 判读修坏（候选不变）

- **R11-2-smoke**（4 对 1 步）：loss 0.6931（=ln2 理论值）、grad_norm 26.5、
  TRL DPOTrainer 接线/渲染/守卫端到端通过。
- **R11-2 正式**（27 对 × 3 epochs = 12 步，dpo-001）：loss 0.6931→0.6636→0.6712
  （守卫通过）、rewards/margins 0→0.061、渲染峰值 624 ≪ 2048、
  determinism.provenance 完整。~9 min GPU。
- **三面评测**（~45 min GPU）：探针 0.4750（−14 = 1.00、放行侧 8 点全 <0.90、
  pv5+premature58）；dev 53/60（pv6）；ood_dev 0.6333（pv3+premature19）。
  **A-6 冻结判读 = 修坏**：候选 sft-008 不变；不重跑不换素材。
- 两次装置拦截面（都是守卫按设计工作）：dpo-001 adapter pin 漏 2 文件
  （sha256sum glob 盲区，补齐）；bank 内容哈希语义（文件字节 sha ≠
  bank_sha256(records)，改同一语义 + 突变测试）。
- CI 修复：82fa8c2 加测后漏跑全量门禁就推送 → 基线算术守卫在 CI 连续 6 红
  → 基线同步 1493/1444+49 + 互斥脚本扩展改 opt-in（默认 v1.0 逐字节可复算）
  → `1d449f5` 恢复全绿。教训已入 Errors 表与本文：**每次推送前必须全量门禁**。

| Date | Command | Result |
|---|---|---|
| 2026-09-06 | R11-1 采样（GPU 0，7396s） | 27 对全在 −14；premise_ok=True |
| 2026-09-06 | B1-0 交叉面构建 + B1-1/2 评测（GPU 0） | base 0.55/pv7 vs sft-008 0.9417/−14=0.125/pv7 |
